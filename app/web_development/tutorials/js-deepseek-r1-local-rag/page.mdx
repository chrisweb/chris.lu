...
modified: 2025-02-26T23:00:00.000Z
...
![A shiny AI bot is crawling through mountains of old paper documents to find a relevant answer to a question](../../../../public/assets/images/app/web_development/tutorials/js-deepseek-r1-local-rag/banner.png 'A shiny AI bot is crawling through mountains of old paper documents to find a relevant answer to a question { banner }')
...
* Download **DeepSeek-R1** using **Ollama** and then run the LLM model locally
...
* Install **PostgreSQL** and **pgvector** to store our embeddings
...
* Find knowledge using a similarity search in our vector database and then add it to the prompt, to provide the AI with information (before it starts reasoning)
...
## Offline DeepSeek-R1 using Ollama
...
In this chapter we will install [PostgreSQL](https://www.postgresql.org/) on our local machine and then add the [pgvector](https://github.com/pgvector/pgvector) plugin. **pgvector** will transform our PostgreSQL database into a vector database. It might not be the fastest solution, but it has the advantage that we only need one database for both our app data as well as the vectors.
...
In this chapter we will use a technique called **Retrieval-Augmented Generation (RAG)** to provide the AI with information from our local (markdown) documents. The goal is to find and then pass additional information to help the AI give better responses.

To achieve this will write a script that will convert our content into embeddings (vectors) and then store them in our vector database.
...
In this tutorial we will use a Node.js version which has [native Typescript support](/web_development/posts/node-js#nodejs-native-typescript-support-type-stripping) to run our script(s) (without having to compile them to javascript first).
...
LangChain is toolset to build RAG pipelines and other workflows powered by an AI model
...
In this tutorial we will re-use DeepSeek-R1 to create high dimensional embeddings, so we don't need to pull an extra model, but if you prefer you could use an embedding model like [NOMIC](https://ollama.com/library/nomic-embed-text), in which case you would first need to pull the model, using the following command: `ollama pull nomic-embed-text:latest`
...
Lines: 89-107: next we initialize the LangChain.js PGVectorStore. We first pass the PostgreSQL connections pool, then we set a name for our new table and then we define which columns we want to have in that table. The last configuration option we set is the distance strategy we intend to use later when do our similarity searches. The most popular strategies are cosine (default), innerProduct and euclidean, in this tutorial we will use cosine (I also did some tests and euclidean performed very well too, but cosine seemed to produce slightly better results, but make sure to experiment a bit with different strategies using your own content and try to find out which one works best for you)
...
Lines: 116-121: We use our **createPool** function to get a pool of connections and use the listener function to track any potential errors (if there is an error it means we cant connect so we exit the script)
...
Lines 22 to 69: we have added the **findKnowledge** function, which will first establish a connection to the database, then run a query that using the cosine distance to find content that is similar to the question of the user. We turned both our content and the user question into embeddings and by using the cosine distance we calculate a score that represents how fow the question is compared to the content. Is the distance big then it means that the content is likely NOT similar and if the distance is small then there is a high chance that the content is similar
...
The results will be different but not automatically better, which means you will have to run a multitude of experiments to be able to find out which one works best
