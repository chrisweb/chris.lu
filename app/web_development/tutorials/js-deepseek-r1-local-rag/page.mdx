...
description: Build a local offline RAG chat app with DeepSeek-R1 and Ollama, a Next.js 15 and React 19 frontend, a JavaScript AI client and backend, and PostgreSQL with pgvector
keywords: ['DeepSeek-R1', 'llama 3', 'ai', 'RAG', 'embeddings', 'pipeline', 'ollama.js', 'LangChain.js', 'PostgreSQL', 'pgvector', 'vector database', 'similarity search', 'Next.js 15', 'React 19', 'local', 'JavaScript', 'TypeScript']
...
<ArticleJsonLd frontmatter={frontmatter} image="/web_development/og/tutorials_js-deepseek-r1-local-rag/opengraph-image" />
...
> [!WARN]  
> This tutorial was written in February 2025 and targets **Next.js 15**, **React 19** and **version 4 of the ai SDK**. The install commands below pin those major versions on purpose. The ai SDK has changed a lot since: from version 5 on, `useChat` no longer returns `input`, `handleInputChange` and `handleSubmit`, messages expose `parts` instead of `content`, `CoreMessage` was renamed to `ModelMessage`, and `toDataStreamResponse` became `toUIMessageStreamResponse`. If you drop the version pins, the code in this tutorial will not work as written
...
Now that our project is open in VSCode we use the **terminal** to run the CNA command (pinned to the Next.js 15 line this tutorial was written for):

```shell
npx create-next-app@15
```

When you get asked if you agree to install that version of CNA, press **y** to accept
...
First we install the [AI SDK](https://sdk.vercel.ai/docs/introduction) (by Vercel), pinned to version 4:

```shell
npm i ai@4 --save-exact
```

We also install the React bindings, which is where the `useChat` hook we are about to use comes from. Version 4 of the ai SDK pulls this package in as a dependency of its own, so the import would resolve even without this step, but relying on someone else's dependency tree is fragile, so we add it explicitly:

```shell
npm i @ai-sdk/react@1 --save-exact
```
...
```shell
npm i ollama-ai-provider@1 --save-exact
```
...
```shell
npm i langchain@0.3 --save-exact
```
...
Lines 23 to 51: we first remove the frontmatter part from markdown documents, adding a block of frontmatter as a chunk is not recommended, which is why we remove the frontmatter
...
#### Extracting frontmatter metadata (optional)

In the previous code example we removed the frontmatter from our markdown content, because passing it as string to the AI would diminish its semantic value. 

A better solution would be to pass the frontmatter string to a parser and then use the metadata to improve the information we will give the AI.

To parse the frontmatter you could install a package like [js-yaml](https://www.npmjs.com/package/js-yaml):

```shell
npm i js-yaml@4 --save-exact
```

and if you use TypeScript add the types:

```shell
npm i @types/js-yaml@4 --save-exact --save-dev
```

Then you edit the embeddings.mts file and first add the new import for js-yaml:

```ts title="scripts/embeddings.mts"
import yaml from 'js-yaml'
```

And then add a custom frontmatter parser to the **chunkNorris** function:

```ts title="frontmatter parser example"
documents.forEach((document) => {
    ...
    if (frontmatter) {
        ...
        // we store the frontmatter in the document metadata
        // you might prefer to do something more useful with it
        // but that would go beyond the scope of this tutorial
...
Finally create a markdown test document:

```md title="/docs/frontmatter-example.md"
---
...
Now run the embeddings script (`npm run embeddings`) and then check what the console log has printed in your terminal
...
```shell
npm i @langchain/community@0.3 pg@8 --save-exact
```

And then the types for pg (node-postgres):

```shell
npm i @types/pg@8 --save-exact --save-dev
```
...
> [!MORE]  
> [pgvector "querying" readme](https://github.com/pgvector/pgvector?tab=readme-ov-file#querying)  
