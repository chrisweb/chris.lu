...
modified: 2025-11-17T23:00:00.000Z
...
### Initialize next-devtools

To trigger the initialization, make sure Github Copilot is set to Agent Mode, choose your favorite model (like Claude Sonnet 4.5), then prompt the Copilot Agent the following sentence:

```txt
Use the next-devtools **init** tool to ensure the Next.js MCP is ready
```

Make sure to do a quick init every time you start a **new chat** session to ensure it is aware of your next-devtools

> [!TIP]
> If you have a custom agent file, or if you have a `.github\copilot-instructions.md` then add the following line to them to make initialization happen automatically for each session:
>  
> ```txt
> ## next-devtools MCP
>  
> - When starting work, ALWAYS call the the **"init" tool** first, it needs to get called once per session. Do this automatically without being asked.
> - Use the **"nextjs_runtime" tool** to query Next.js runtime information, diagnostics and internals.
> - Use the **"nextjs_docs" tool** to look up Next.js documentation when needed.
> - Use the **"browser_eval" tool** to use use playwright to interact with the web application in a browser-like environment for testing and debugging purposes.
> ```
...
```mjs title="next.config.ts" showLineNumbers {8-9}
...
### Initialize the next-devtools MCP

Make sure the next-devtools are initialized by sending the following prompt to the agent:
