...
As an alternative there is [eslint-plugin-tailwindcss](https://www.npmjs.com/package/eslint-plugin-tailwindcss) which works great with tailwind v2/3 but I had problem with v4 not finding my css file on windows. They have a first beta with [tailwind v4 support](https://github.com/francoismassart/eslint-plugin-tailwindcss/releases/tag/v4.0.0-beta.0) and you can track the progress in their [v4 repository](https://github.com/francoismassart/eslint-plugin-tailwindcss/tree/alpha/v4).

### eslint-plugin-better-tailwindcss

> [!NOTE]  
> We are using eslint-plugin-better-tailwindcss **v4**, if you had this plugin installed before and are migrating to v4, then have a look at the [v4 changelog](https://github.com/schoero/eslint-plugin-better-tailwindcss/blob/v4/CHANGELOG.md#v400-beta0) as there are some breaking changes.
...
> [!NOTE]
> Note the @beta at the end of the package name. We are installing the beta version. A stable release is expect to ship before the end of 2025. I am currently using the beta and had no problems so far.
