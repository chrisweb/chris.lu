...
For example, to enable ESLint cache, we need to use the `--cache` flag, and then we tell ESLint where to store that cache using the `--cache-location` flag, we use a `.eslintcache` file in the root of the project (ESLint's own default location, so clearing the `.next` build output does not throw the lint cache away with it), remember to add `.eslintcache` to your `.gitignore`
...
```js title="package.json" showLineNumbers{5} {5}#special {6}
  "scripts": {
    ...
    "start": "next start",
    "next-lint": "next lint",
    "lint": "eslint ./ --ext .js,.jsx,.ts,.tsx,.mdx,.md --cache --cache-location .eslintcache"
  },
```
