---
title: Sitemap (google search console and bing webmaster tools), robots.txt and manifest file in Next.js - Tutorial
description: Sitemap (google search console and bing webmaster tools), robots.txt and manifest file - Next.js static MDX blog | www.chris.lu Web development tutorials
keywords: ['Sitemap', 'robots.txt', 'manifest', 'seo', 'google search console', 'bing webmaster tools']
published: 2024-08-26T18:18:18.444Z
modified: 2024-09-10T18:18:18.444Z
...
You can put a static `manifest.webmanifest` file into the `/app` folder. To create a static manifest you could for example use an [online generator](https://manifest-gen.netlify.app/) or reuse the tool [realfavicongenerator.net](https://realfavicongenerator.net/) (we already saw when creating a favicon), as it does not only generate a favicon, the downloaded package will also contain a manifest

If you prefer you can rename the manifest file to something else than `manifest.webmanifest`, the [specs](https://w3c.github.io/manifest/#using-a-link-element-to-link-to-a-manifest) specify that any name is valid and that the extension needs to be `.webmanifest`, there is one exception which the .json extension, you could use `manifest.json` as filename like for example, GitHub does: [https://github.com/manifest.json](https://github.com/manifest.json)
...
Now, make sure the dev server is running (`npm run dev`) and then open [http://localhost:3000/](http://localhost:3000/) in your browser, then Press `F12` or right-click and then select **Inspect** to have a look at the elements inside of the `<head>`
