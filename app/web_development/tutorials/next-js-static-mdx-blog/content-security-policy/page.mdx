...
> [!TIP]  
> If you want to debug your code, you might want to also add the reporting for development. In that case, add the `${reportCSPViolations}` and `${reportingDomainWildcard}` variables to the development directives too (same as for preview and production) and then check out the chapter about [disabling the "reports from localhost"](/web_development/posts/sentry-io/#disable--enable-reports-from-localhost-filter) filter in my Sentry.io post as you will need to disable the filter for localhost reporting in your Sentry configuration on Sentry.io  
>  
...
```js title="next.config.mjs" showLineNumbers{107} {1-11} {14} {19-30}
