...
To add MDX 3 support to the Next.js blog, I will use [@next/mdx](https://www.npmjs.com/package/@next/mdx), which is an MDX package by the same team that is behind Next.js, but other alternatives work well too, I listed some of them in the [@next/mdx alternatives chapter](/web_development/posts/mdx#nextmdx-alternatives) in my MDX post
...
> 
> Also important if you chose to put all your files into an `./src` folder, then you need to put the `mdx-components.tsx` file into the `./src` folder and not the root of the project
...
Line 1: we import the `@next/mdx` package (that we installed in the previous chapter)

Lines 5 to 12: we configure **@next/mdx** by passing an object to **createMdx**, using the **extension** property, we tell @next/mdx what for what pages it should use the MDX parser, and via the options, we add support for **remark plugins** and **rehype plugins** (we will add a bunch of those later)

Lines 25 to 26: we tell Next.js what extensions are valid page extensions, the pages we created so far had a `.tsx` extension, but the new MDX pages we will now add will have the `.mdx` extension, as you can see we add a bunch more here to cover all use cases if, for example, you are sure you won't use `.js` as an extension that you can remove it
...
The configuration we just did adds support for MDX in pages that have the extension `*.mdx`, if you want also to add support for pages with the `*.md` extension, then add the extension in two places like so:
...
If you use another extension for your markdown pages, like *.markdown, *.mdown, *.mkd, ... then replace the **md** with the extension you prefer using
...
Also, if you don't add your markdown extension to the **pageExtensions**, then the Next.js app router will not be able to find your file, and it will show a 404
...
Unfortunately, the documentation does not clearly state that **plugins do NOT work if you turn on mdxRs** (yet), then they fixed the docs [Docs: Note that needs to turn off mdxRs when using MDX plugins](https://github.com/vercel/next.js/issues/47098) but shortly after they removed the warning again 😭
...
Because we used **create next app** in our tsconfig.json, we **already** have the following included in our `tsconfig.json` (if it isn't in your tsconfig.json, then you need to add it), which means that `*.d.ts` files will get included when typescript does the compilation:
