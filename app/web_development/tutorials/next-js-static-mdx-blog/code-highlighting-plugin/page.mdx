...
Lines 3 to 9: we add a code block example to our playground
...
Lines 180 to 183: we use a selector to target all lines that have a `data-highlighted-line` attribute, we change the background color of the line as well as the color of the new border (vertical line) we added on the left
...
Line 1: we add a new highlight style ID after the curly brackets defining which line(s) should get highlighted, and we set the highlight line ID to `errorLine`
...
Line 1: we remove the curly brackets (used to highlight a line) from the previous example and instead used two slashes (`/`) to tell rehype pretty code that we want to highlight `helloWorld()`
...
Lines 16 to 18: we set the rehype pretty code `defaultLang` configuration option for a code `block` to `js`
...
Line 1: we remove the language flag (and any other options)
