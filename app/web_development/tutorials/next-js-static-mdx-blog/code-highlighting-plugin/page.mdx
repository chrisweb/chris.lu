...
For me, the best of **rehype pretty code** is that it uses [shiki](https://shiki.matsu.io/) under the hood, **shiki** is fantastic at highlighting code, but it also comes with an impressive feature, which is that you can use your favorite VSCode theme to make your code blocks look the same as your code in VSCode
...
The markdown code block syntax got converted into \<pre> HTML element, and inside of that container, there is a \<code class="language-js"> HTML element, but there is no colorful syntax highlighting yet
...
When it comes to styling and using themes, you have the option to either keep the original background color that is included in the theme or deactivate it
...
As we will see, it is not difficult to use a custom color

However, finding one that looks good and still works well with colors used to highlight the code can be tricky
...
Another great feature of rehype pretty code is that you can highlight code lines, and it is effortless to do
...
Lines 174 to 178, we use a selector to target all lines, lines have a `data-line` attribute, and the CSS we add will border on the left, which will create a vertical line that we will then be able to colorize when the line is highlighted but by default we set it to `transparent`, to create this vertical line is of course not mandatory, if you don't like it leave it away

Line 180 to 183, we use a selector to target all lines that have a `data-highlighted-line` attribute, we change the background color of the line as well as the color of the new border (vertical line) we added on the left
...
If you want more than one highlighted line style, then you can use IDs to create an unlimited number of variations
...
Lines 185 to 188, we added a selector to target the `data-highlighted-line-id` attribute if it has the value `errorLine`, we set a reddish color for the background and vertical line to make this code block line look like a code line with an error
...
Line 1, we removed the highlight line curly brackets from the previous example and instead used two slashes (`/`) to tell rehype pretty code that we want to highlight `helloWorld()`
...
Line 194 to 196, we set the background color for highlighted characters when using the **mySpecialHighlight** ID
...
So far, we have always used javascript by adding the language flag `js`, but if you want, for example, to change the programming language from **javascript** to **typescript**, then all you need to do is change the language flag (after the backticks) to `tsx`, like so:
...
There is a second option to specify the programming language, and this option is helpful if all your code blocks contain code written in the same language, and you don't want always to have to set the language flag for every code block
...
If you want to remove the linting warning, use a [comment to disable the remark-link rule](#remark-lint-disable-comments-in-mdx) for that code block or if you want to disable that remark-link rule altogether, then you need to edit your `.remarkrc.mjs` file and set the rule to false, as we previously did in the [configuring remark-lint](#adding-and-configuring-remark-lint) chapter, after editing the remark-lint configuration you might want to [restart the eslint server in VSCode](#restart-the-eslint-server-in-vscode) and [delete the eslint command cache](#clear-the-eslint-cache)
...
To showcase this, let's edit our playground to add an inline code example like so:
...
As you might notice, if you run the dev server and then inspect the HTML in the browser, that rehype pretty code did not get applied, and instead, we still have a regular `<code>` element

As we just saw in the previous chapter, rehype pretty code needs some information to know what colors to apply

We also saw in the previous example that we have 2 options to specify the programming language for a code block
...
Both code blocks and inline code use similar HTML elements, but there is one significant difference, which is that for code blocks, there is a `<pre>` element between the `<figure>` and the `<code>` element, but inline code has no `<pre>` element, we are using this to our advantage here to be able to apply styling only to inline code, by using a [CSS selector](https://developer.mozilla.org/en-US/docs/Learn/CSS/Building_blocks/Selectors) that will only apply our styling if the `<code>` element is a direct child of the `<figure>` element by using a child combinator (`>`), the CSS we add is nothing special, we add a small border radius, some padding and the same background-color we previously used for the code blocks `<pre>` element
...
If you launch the dev server and have a look at the playground page in your browser, you will see this works

Our inline code has been highlighted, and the colors are correct

The `variable` now has the same color as our variables in the code block above, and so does the `hello-world()` function
...
Now, the color is the one we would expect it to be for a function
...
Another way to find out what tokens your theme uses is to look at the json file of the VSCode theme you are using
...
Line 20 to 28, we add several custom token aliases to our rehype pretty code configuration, I tried to use abbreviations that are easy to remember without having to check out the configuration file constantly
...
This section contains a list of token aliases you can use for inline code in markdown (MDX) files
