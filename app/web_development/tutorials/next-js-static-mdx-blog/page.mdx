...
React and Next.js have evolved a lot lately, the most notable additions are **server components** and **(server) actions**. With those new features came a lot of changes for how we devs build Next.js apps / websites, which is why I did a seperate blog post ["The road to React 19 and Next.js 14"](/web_development/posts/road-to-react-19-next-js-14) where I list what changed during the last +-2 years in the React and Next.js ecosystem. If you did not build a project with Next.js 13/14 and React 18/19 yet I recommend you check it out, but if you are already an expert feel free to skip it 😉

> [!MORE]  
> [chris.lu "The road to React 19 and Next.js 14" post](/web_development/posts/road-to-react-19-next-js-14)
...
Then line 14 to 17 we use a template literal to create the reporting uri directive, this directive will tell the browser what URL it should use when sending the CSP reports

Next line 46 and also line 61 we have added the **reportingDomainWildcard** to the connect-src directive

Finally line 49 and 64 we **reportCSPViolations** variable which contains reporting directive
...
In the `app` folder, create a new `tutorial_examples` folder and then in it another `first_page` folder

Then inside of the `first_page` folder add a `page.mdx` (note that we set **extension** to **mdx** and NOT tsx) and in it paste the following content:

```md title="/app/tutorial_examples/first_page/page.mdx"
# Hello 👋 with MDX!
...
And then visit your newly created MDX page in the browser at `http://localhost:3000/tutorial_examples/first_page`

Congratulations you just learned how to create MDX pages 🎉 
...
The empty circle (`○`) in front of our `/tutorial_examples/first_page` indicates that Next.js will statically generate routes (pages) at build time instead of on-demand at request time
...
Now lets edit the `/app/tutorial_examples/first_page/page.mdx` page we created earlier and add our Counting component to it, like so:

```mdx title="/app/tutorial_examples/first_page/page.mdx"
...
As we saw in the previous chapter you can add classes and use the id in CSS Modules, but there is one thing you can't use in CSS Modules and that's the Element type

Let's (re-)open the CSS Module we used for our button and styling for the button by using the Element type, like so:
...
:root {
    ...
    /* light color scheme */
    --background-light-value: 19 71% 97%;
    --background-light-color: hsl(var(--background-light-value));
    --text-light-value: 288 23% 25%;
    --text-light-color: hsl(var(--text-light-value));
    --primary-light-value: 315 31% 32%;
    --primary-light-color: hsl(var(--primary-light-value));
    --secondary-light-value: 212 33% 33%;
    --secondary-light-color: hsl(var(--secondary-light-value));
    /* dark color scheme */
    --background-dark-value: 288 23% 25%;
    --background-dark-color: hsl(var(--background-dark-value));
    --text-dark-value: 19 71% 97%;
    --text-dark-color: hsl(var(--text-dark-value));
    --primary-dark-value: 315 30% 82%;
    --primary-dark-color: hsl(var(--primary-dark-value));
    --secondary-dark-value: 211 33% 83%;
...
#### website / app color palette

In the global.css we just created, I chose some colors, but you probably want to chose use a different color palette, but if like me you are no expert in picking colors that look good together to create a color palette for your website or app, then here is a trick I often use

I first try to find an image that has a mix of colors that I find appealing and then I use an [image to color palette tool](https://coolors.co/image-picker), that will pick colors from an image and create a palette for me

![banner showing the coolors.co image to color palette tool](../../../../public/assets/images/app/web_development/tutorials/next-js-static-mdx-blog/image_to_color_palette.png)

After uploading the image the tool will suggest a palette based on the colors in the image

1. Click on **Browse Image** and upload the image you like
1. Using the **slider** on top to make the tool pick different colors and suggest different palettes
1. Ajust the picked color manually by clicking on the circle(s) and then dragging it to desired spot
1. Use the **+** / **-** to adjust the amount of colors you want to have in the palette
1. After you have found a palette you like, click on the **down arrow** next of the **Export palette** button, then chose **Quick view** and then click on any of the colors, the tool will offer you a wide range of formats, from HEX to RGB or HSL, click on the format you prefer to copy it, you could also just click on **Export palette** button itself (instead of the down arrow) which will open a modal and give you several options to copy paste, however not all formats are there, for example if you like to use RGB

Another nice tool to create color palettes is the free online tool called [palettte.app](https://palettte.app/), if you want to give it a try I recommend first reading the [tutorial](https://gabrielschneider.de/palettte-app/)

> [!MORE]  
> [coolors.co "image colors picker"](https://coolors.co/image-picker)  
> [MDN "CSS colors" documentation](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_colors)  
> [palettte.app](https://palettte.app/)  

##### Hue, Saturation, Lightness (HSL) colors

You might be wondering what color format you should use? Like colors themself, what ever you prefer is best but if you have no preference use HSL. What is nice about HSL because it allows you to quickly make variants of a color by manually tweaking the values, for example you want to use one of your colors but 10% lighter then you can just change the third value of your HSL color to 10% less. This makes it easy to create a whole range of shades for a given color:

```css
:root {
    /* 70% lighter than base */
    --lighter-color: hsl(217 100% 70%);
    /* 60% lighter than base */
    --light-color: hsl(217 100% 60%);
    /* base */
    --base-color: hsl(217 100% 50%);
    /* 40% darker than base */
    --dark-color: hsl(217 100% 40%);
    /* 30% darker than base */
    --darker-color: hsl(217 100% 30%);
}
```
...
<div style={{
    ...
    <div style={{ backgroundColor: 'hsl(217 100% 70%)', width: '100px', height: '100px' }}>lighter</div>
    <div style={{ backgroundColor: 'hsl(217 100% 60%)', width: '100px', height: '100px' }}>light</div>
    <div style={{ backgroundColor: 'hsl(217 100% 50%)', width: '100px', height: '100px' }}>base</div>
    <div style={{ backgroundColor: 'hsl(217 100% 40%)', width: '100px', height: '100px' }}>dark</div>
    <div style={{ backgroundColor: 'hsl(217 100% 30%)', width: '100px', height: '100px' }}>darker</div>
</div>

> [!MORE]  
> [MDN "hsl()" documentation](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value/hsl)  
> [caniuse "hsl() (HSL color model)" page](https://caniuse.com/mdn-css_types_color_hsl)  

##### color code converter(s)

If working with colors and you need to quickly convert a color from HEX to RGB or HSL but don't want to open a heavy paint tool, just type ["color picker"](https://www.google.com/search?q=color+picker) into google search, then paste your hex or rgb or hsl color code into the picker or just use the sliders to chose a color, then the picker will show you that color in several formats, now you just need copy the format you want to use

Another easy and quick way of converting colors from one format to another is using the developer tools, for example in chrome after opening the developer tools (using the `F12` key or by right clicking into the page and then selecting **Inspect**), open the **Elements** tab, then on the right you can see the css of the page, when there is a color in the css the developer tools will show a small rectangle with that color inside, click on that rectangle to open the color picker

In the color picker you have an icon that consists of two angle brackets (pointing up and down) which has a tooltip that says **Change color format**:

![picture showing the chrome developer tools color picker icon](../../../../public/assets/images/app/web_development/tutorials/next-js-static-mdx-blog/chrome_dev_tools_color_picker_change_format_icon.png)

Click on it to open a menu with a list of alternative formats and chose the one you want to use:

![picture showing the chrome developer tools color picker change format menu](../../../../public/assets/images/app/web_development/tutorials/next-js-static-mdx-blog/chrome_dev_tools_color_picker_change_format_menu.png)

The last option in the color formats list is the CSS [color()](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value/color) functional notation that allows you to switch to another [color space](https://developer.mozilla.org/en-US/docs/Glossary/Color_space) other than the default sRGB color space, be careful though when using the CSS color() function as it got introduced in the [W3C "CSS Color Module Level 4" specification](https://drafts.csswg.org/css-color/), meaning the browser support is still quite low as you can see on the [caniuse "color()" page](https://caniuse.com/css-color-function)

> [!MORE]  
> [google.com "color picker" widget](https://www.google.com/search?q=color+picker)  
> [developer.chrome.com "color picker" documentation](https://developer.chrome.com/docs/devtools/css/color)  

##### color contrast

The most important thing when chosing colors for you websites / app is to make sure the contrast between background and text colors is hight enough

To check if your contrast is high enough, there is a very nice free [Color Contrast Checker](https://coolors.co/contrast-checker/fdf5f2-385270) online tool you might want to try, just paste the color you intend to use as background color as well as the color you intend to use for your text and the tool will tell you how good the contrast is, a good contrast starts at 3, however if you want to have really high contrast a value above 7 is recommended

If you want to know more about why color contrast matters and why it is an important accessibility feature then I recommend reading the [W3C "contrast" accessibility guidelines](https://www.w3.org/WAI/WCAG21/Understanding/contrast-minimum.html)

> [!MORE]  
> [coolors.co "Color Contrast Checker"](https://coolors.co/contrast-checker/fdf5f2-385270)  
> [W3C "contrast" accessibility guidelines](https://www.w3.org/WAI/WCAG21/Understanding/contrast-minimum.html)  
...
```tsx title="/app/layout.tsx" showLineNumbers {3} {19}#special
import './global.css'
import { Metadata } from 'next'
...
The most important information is the first row of the logs, where it says that the **Blog** page got fetched using a **GET** request (I will explain in a bit why we did this initial test)
...
Have a look at the first line (actually also second line because there are now two rsc fetches being made), remember last time when we used the anchor we had a page request on the first line loading the entire Blog **document**, but this time we just have two **fetch** requests getting some RSC (react server component) data from the server

As you can see the difference is huge when using the next/link component compared to using a regular anchor element, the exact values don't matter this is just simple example, but as you can the difference between the values on the first screen and the second are substancial

When you use next/link Next.js will only fetch the data it needs instead of loading the full page, for example the layout has not changed between the homepage and the blog page so Next.js won't fetch it again

Faster loading times are of course important because the user will be able to navigate faster and if an app is fast the user is happy, but it also great because your server running Next.js will spend less time building pages and responding to requests which means it will be able to handle more requests and finally faster loading times will also improve your SEO score
...
!!! demonstrate how not using next/link will cause the browser to fetch whole page: [https://github.com/chrisweb/react-nextjs-nodejs-typescript-grapql-tutorial/blob/386a0e15224f9a34b34e284c36097a3373ab9faf/documentation/SSR_AND_CSR.md?plain=1#L12]()


In this chapter we will create a react navigation component for our Next.js project
...
First lets create a new test playground to test the components, inside of the `/app/tutorial_examples` folder create a `mdx-components_example` folder

Then in the `mdx-components_example` folder create a `page.mdx` file and paste the following content into it:

```mdx title="/app/tutorial_examples/mdx-components_example/page.tsx"
# headline 1
