...
> [!WARN]  
> There is one thing that however does not work, using remark-gfm and the following syntax `[ ]` and `[x]` is supposed to get rendered as HTML input fields of type checkboxe:
>  
> ```md
> [ ] checkbox
> ```
>  
> I have NOT been able to get this to work (as of now), so be aware that checkboxes and footnotes might not work for you too, if you have any idea what causes them to not render as checkboxes please let me know in a [chris.lu repository discussion](https://github.com/chrisweb/chris.lu/discussions) and I will update this tutorial with a note or fix
