...
Line 64 to 40 same as for the `<main>` element we again set the **display** property to **flex** and the **flex-direction** to column to make sure all elements inside of our `<article>` element get displayed one under the other, then we add a padding using our `--spacing` CSS variable to 16 pixels on each side, because we also added maximum width to the main element this means that article content (if we don't have an aside) will be a maximum 1120 - 32 = 1088 pixels wide, this will matter in a later chapter when we start adding images to our cotnent, finally we set the width to 100% to make sure that the article always uses the full width of the parent main element

Line 72 to 78 we add some color to all our links using the CSS `var()` function to use the variables we set earlier in the  and we also use the [:hover](https://developer.mozilla.org/en-US/docs/Web/CSS/:hover) pseudo class to change the color of links on hover

Line 80 to 108 we add use  the font-size and line-height properties to define the height of all our headings (like any other values in this CSS stylesheet, those are just my examples, feel free to tweak those values to your liking)
...
```css title="/app/global.css" showLineNumbers{112} {1-8}
.iconSmall {
...
Line 112 to 119 we add some CSS to style our icon:
...
```css title="/app/global.css" showLineNumbers{121} {1-6} {8-11} {13-15}
...
Line 121 to 126 we just reset the margin and padding on the figure element that is the container of a **rehype-pretty-code** code block, it has a **data** attribute that we use to target it as well as the **pre** and **code** element inside of it

Line 128 to 130 we set its **width** to **100%** to make sure our code block is as wide as our articles (**rehype-pretty-code** adds a container around code blocks, which consists of a `<figure>` element width a **data** attribute that has the value **rehype-pretty-code-figure**, this is what we target using the `data-rehype-pretty-code-figure` selector) and then we set the **overflow** property to **auto**, meaning that if the code inside of the code block is too large the code block is allowed to add (and display) scrollbars, if the code is fits the code block then the scrollbars should be hidden

Line 133 to 135 we add a padding to the `<pre>` element that is inside of the `<figure>` container
...
```css title="/app/global.css" showLineNumbers{137}
...
```css title="/app/global.css" showLineNumbers{162}
[data-line] {
...
Line 162 to 166 we use a selector to target all lines, lines have a `data-line` attribute, the CSS we add will border on the left which will create a vertical line that we will then be able to colorize when the line is highlighted but by default we set it to `transparent`, to create this vertical line is of course not mandatory, if you don't like it just leave it away

Line 168 to 171 we use a selector to target all line that have a `data-highlighted-line` attribute, we change the background color of the line as well as the color of the new border (vertical line) we added on the left
...
```css title="/app/global.css" showLineNumbers{173}
...
Line 173 we added a selector to target the `data-highlighted-line-id` attribute if it has the value `errorLine` and on the next we added a more reddish color for the background and vertical line to make this line appear like a line with an error
...
```css title="/app/global.css" showLineNumbers{178}
...
```css title="/app/global.css" showLineNumbers{182}
...
```js title="next.config.mjs" showLineNumbers {16}
import { PHASE_DEVELOPMENT_SERVER } from 'next/constants.js'
import createMdx from '@next/mdx'
import rehypeMDXImportMedia from 'rehype-mdx-import-media'
import rehypePrettyCode from 'rehype-pretty-code'
...
Line 16 we set the rehype pretty code `defaultLang` configuration option for a code `block` to `js`
...
Line 16 we removed the `js` language flag
...
As you might notice if you run the dev sever and then inspect the HTML in the browser is that rehype pretty code did not convert the inline code block to a highlighted code block

As we just saw in the previous chapter rehype pretty code needs some information to know what colors to apply, we also saw in the previous example that we have 2 options the specify the programming language for a code block, for inline code blocks we have one more option:

* add a language flag
* set a default language in the configuration
* add a token (this is an extra option only inline code blocks can use)

First lets add a bit of CSS to our global.css to improve the styling of inline code blocks:

```css title="/app/global.css" showLineNumbers{186}
...
Both code blocks and inline code use similar HTML elements, but there is one major difference which is that for code blocks there is a `<pre>` element between the `<figure>` and the `<code>` element but inline code has no `<pre>` element, we are using this to our advantage here to be able to apply styling only to inline code, by using a [CSS selector](https://developer.mozilla.org/en-US/docs/Learn/CSS/Building_blocks/Selectors) that will only apply our styling if the `<code>` element is a direct child of the `<figure>` element by using a child combinator (`>`)
...
```js title="next.config.mjs" showLineNumbers {17}
import { PHASE_DEVELOPMENT_SERVER } from 'next/constants.js'
import createMdx from '@next/mdx'
import rehypeMDXImportMedia from 'rehype-mdx-import-media'
import rehypePrettyCode from 'rehype-pretty-code'
...
Line 17 we add the `inline` option inside of the `defaultLang` configuration for rehype pretty code and set its value to `js`
...
```js title="next.config.mjs" showLineNumbers{12}
    /** @type {import('rehype-pretty-code').Options} */
    const rehypePrettyCodeOptions = {
        theme: JSON.parse(themeFileContent),
        tokensMap: {
...
Then we add code block and a list of inline code examples to our playground page to test our tokens map:
