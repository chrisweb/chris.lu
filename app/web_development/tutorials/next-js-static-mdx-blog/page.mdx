...
Next we start the dev server and have look at how our code block gets displayed, you will see that the markdown code block syntax got converted into an html \<pre> tag with an \<code class="language-js"> html tag inside, but there is no colorful syntax highlighting yet, this is why we will now add the *rehype pretty code* plugin
