...
In the previous example(s) we did set the programming languge to **javascript** using the `js` language flag (after the 3 backticks of our fenced code block), because rehype pretty code needs that information to know what colors to use when highlighting our code

There is a second option to set the language, which is interesting if all your code blocks contain code written in the same language and you don't want to always have to set the language flag for every code block
...
Line 15 we set the rehype pretty code `defaultLang` configuration option for a code `block` to `js`
...
However if you did follow the ESLint with remark lint setup we did earlier in this tutorial then you should see that now our code block is underlined and if you hover over it you will see it is an ESLint warning:
...
Some text `variable` some more text `function()` even more text

</article>

````

As you might notice if you run the dev sever and then inspect the HTML in the browser is that rehype pretty code did not convert the inline code block to a highlighted code block, as we just saw in the previous chapter rehype pretty code needs some information to know what colors to chose

If you want rehype pretty code to convert an inline code block to a highlighted one you have 3 options:

* add a language flag
* add a token (this is an extra option only inline code blocks can use)
* set a default language in the configuration

Next we add a bit of CSS to our global.css to improve the styling of inline code blocks:
...
If we tell rehype pretty code what programming language the code inside of the code block is, then the code block highlighting colors will get adjusted, so far we always used javascript by adding the language flag `js`, if you want to use **typescript** instead then you can switch the language flag that is behind the first backticks to for example `tsx`, like so:
...
> [!TIP]  
> You can use language aliases like js and jsx, ts and tsx and also md and mdx, but for example mjs or esm are not a supported aliases, you can check out the full list of languages / aliases by looking at the [shiki languages file](https://github.com/shikijs/shiki/blob/main/packages/shiki/src/assets/langs-bundle-full.ts#L1248)

For inline code the language hint needs to be inside curly brackets at the end of our inline code, like so:

```mdx
Some text `variable{:js}` some more text `function(){:ts}` even more text
```
...
now even though we specified the language, the color is wrong, this is because based on the little bit of code the highlighter can NOT know that it is actually a function and instead assumes it is a variable
...
Now the color is the one we expected

You might wonder how do you know what each and every token is, well at the top of this chapter we imported a vscode json theme, in the json files of the VSCode there is a section called **tokenColors** and in that list are all the tokens and their color settings
...
> [!MORE]  
> ["rehype pretty code" website & documentation](https://rehype-pretty-code.netlify.app/)  
