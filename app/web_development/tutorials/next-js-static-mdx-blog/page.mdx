...
```js title="next.config.mjs" showLineNumbers {2} {6-13} {34-35}
...
Line 2 we import the `@next/mdx` we installed the previous chapter

Line 6 to 13: We configure **@next/mdx** by passing an object to **createMdx**, using the **extension** property we tell @next/mdx what for what pages it should use the MDX parser and the via the options we add support for **remark plugins** and **rehype plugins** (we will add a bunch of those later)

Line 34 to 35 we tell Next.js what exntensions are valid page extensions, the pages we created so far had a `.tsx` extension but the new MDX pages we will now add will have the `.mdx` extension, as you can see we add a bunch more here to cover all use cases, if for example you are sure you won't use `.js` as extension that you can remove it
...
```js title="next.config.mjs" showLineNumbers {8-10} /[rehypePrettyCode, rehypePrettyCodeOptions]/#special
import { PHASE_DEVELOPMENT_SERVER } from 'next/constants.js'
import createMdx from '@next/mdx'
import rehypeMDXImportMedia from 'rehype-mdx-import-media'
import rehypePrettyCode from 'rehype-pretty-code'
...
Line 8 to 10 we create a **rehype-pretty-code** options object and tell shiki that we want to use the bundled [Dracula Theme for Visual Studio Code](https://draculatheme.com/visual-studio-code)

Line 17 we add the options to our rehype plugins configuration
...
#### Using VSCode themes by getting the git repository

If there is a theme you wish to use and it is hosted on GitHub (and not among the ones bundled in shiki) then you can do that too

First go to the git repository of the theme and copy the **https** URL, then install it using the npm install command but instead of using a package name you use the repository URL, for example to install [One Dark Pro (Atom's iconic One Dark theme for Visual Studio Code)](https://github.com/Binaryify/OneDark-Pro/tree/052d61e1aeab15b15396f6e958c166950670a1d1/themes) you would use the following command:
...
```js title="next.config.mjs" showLineNumbers {5} {9} {10} /[rehypePrettyCode, rehypePrettyCodeOptions]/#special
import { PHASE_DEVELOPMENT_SERVER } from 'next/constants.js'
import createMdx from '@next/mdx'
import rehypeMDXImportMedia from 'rehype-mdx-import-media'
import rehypePrettyCode from 'rehype-pretty-code'
...
const nextConfig = (phase) => {
    ...
    const themeJsonPath = new URL('./node_modules/material-theme/themes/OneDark-Pro.json', import.meta.url)
...
Line 10 we use the **readFileSync** to read the content of the theme colors file

Line 13 we parse the theme json file content using the javascript **JSON parser**

#### Parsing VSCode themes that use yaml instead of json

Not all VSCode themes can be used out the box with shiki, if for example you want to use one that yaml files to configure the VSCode colors, like the [Dracula for Visual Studio Code](https://github.com/dracula/visual-studio-code/tree/e475d548db27773fa0004b252c0a4701f187fb7e/src) (you can uss this one like we did in beginning as it is bundled with shiki but I use it here as an example to show you what to do when the theme uses yaml files), in this cas you need to include a yaml loader to your setup

> [!NOTE]  
> We will now install the [yaml package](https://www.npmjs.com/package/yaml), there is another package called [js-yaml](https://www.npmjs.com/package/js-yaml), which had more stard but no recent updates, it would probably do the job too but I chose to use the **yaml** package instead as it has a lot of stars and downloads too, as well as recent updates

To install the **yaml** package use the following command:

```shell
npm i yaml --save-exact
```
