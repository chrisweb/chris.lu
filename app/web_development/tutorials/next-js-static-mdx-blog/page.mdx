...
```mjs title="next.config.mjs" showLineNumbers {3-5}
...
```mjs title="next.config.mjs" showLineNumbers {1} {3} {12}#special
...
Line 10 we check if the phase is **development** server by checking if the content of our phase variable is equal to **PHASE_DEVELOPMENT_SERVER**, if it is we enable typedRoutes and if it is NOT we disable the typedRoutes option
...
```mjs title="next.config.mjs" showLineNumbers {8}#special
import { PHASE_DEVELOPMENT_SERVER } from 'next/constants.js'
...
```mjs title="next.config.mjs" showLineNumbers {9}#special
import { PHASE_DEVELOPMENT_SERVER } from 'next/constants.js'
...
### Our 1st typescript (javascript) page
...
Line 7 to 69 we have added a quite long function **cspHeader()** which will create 4 sets of CSP rules:
...
```js title="next.config.mjs" {7-8} {14-17} {46}#special {60}#special
// CSP headers here is set based on Next.js recommendations:
// https://nextjs.org/docs/app/building-your-application/configuring/content-security-policy
const cspReportOnly = true;

const reportingUrl = 'https://xxxxxxxxx.ingest.sentry.io/api/0000000000/security/?sentry_key=yyyyyyyyyyyy'
...
Then line 17 to 17 we use a template literal to create the reporting uri directive, this directive will tell the browser what URL it should use when sending the CSP reports

Next line 46 and also line 60 we have added the reportingDomainWildcard to the connect-src directive
...
```js title="next.config.mjs" showLineNumbers{79} {3-13} {31}#special {36-47}
const nextConfig = (phase) => {
...
Line 3 to 13 we added a new variable `extraSecurityHeaders` to store the HSTS header as we want to exclude it in development where we don't have an SSL certificate
...
Line 31 we use the `extraSecurityHeaders` to add the `Strict-Transport-Security` header to the list of headers

Line 36 to 47 we add the 3 security headers to the list of headers:
...
Let's start with a simple example, in the `components` folder start by creating a new folder `UI`

Next in the `UI` folder we create a new `button.module.css` file and add the following content:

```css title="/components/UI/button.module.css"
.reset {
    background: none;
    border: none;
    padding: 0;
...
Then we open our homepage (the page in the app folder) and import /use the button:
...
```tsx title="/components/UI/Button.tsx" "&#x60;${styles.reset} ${styles.core}&#x60;"
'use client'
...
```css title="/components/UI/button.module.css" {14-16}
.reset {
    background: none;
    border: none;
    padding: 0;
