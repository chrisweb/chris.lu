...
Rehype pretty code will also highlight inline code
...
As you might notice if you run the dev sever and then inspect the HTML in the browser is that rehype pretty code did not convert the inline code block to a highlighted code block, for rehype pretty code to do something you need to add a language hint, a token or set a default language in the configuration
...
If we tell rehype pretty code what programming language the code inside of the code block is, then the code block highlighting colors will get adjusted, so far we always used javascript by adding the language hint `js`, if you want to use **typescript** instead then you can switch the language hint that is behind the first backticks to for example `tsx`, like so:
