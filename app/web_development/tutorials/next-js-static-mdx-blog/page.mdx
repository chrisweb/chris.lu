...
> [!WARN]  
> There seems to be a bug in the route types for `*.mdx` pages, when doing a prod build, when doing a dev build all routes are typed and listed in the `.next/types/links.d.ts` but they are missing in prod resulting in a **Type error** "XY is not an existing route.", so even though this feature seems very promissing, as of now I recommend to keep it turned off (set to false) if you have are using `*.mdx` pages (which is what we will do in this tutorial)
...
```js title="next.config.mjs" showLineNumbers {1-61} {70-82}
...
```js title="next.config.mjs" showLineNumbers{51} {56}#special
    // for dev environment enable unsafe-eval for hot-reload
    return `
        ${defaultsCSPHeaders}
        font-src 'self';
...
```js title="next.config.mjs"{63} showLineNumbers {76}
/** @type {import('next').NextConfig} */
const nextConfig = {
    poweredByHeader: false,
    experimental: {
...
Depending on the value of **cspReportOnly**, we either set the CSP header to **Content-Security-Policy-Report-Only** mode (if true) or **Content-Security-Policy** (if false)
...
For now we set the CSP mode to only report violations, however as soon as are confident that there are no more violation reports it is recommend to set our custom variable **cspReportOnly** to **false**
...
### Logging CSP violations with Sentry.io

CSP violations reports can not only be seen in the console, as this would limit us to only see violations that occur when we visit our app, they can also be logged

You could write your own API to log CSP violations or you can use a service like [Sentry.io](https://sentry.io), we will do the later and create a free Sentry account, we will use that account to log the CSP violations and also to monitor error and loading times when our users visit app
...
Now we need create a new project on Sentry.io and use their wizard to include their Sentry.io SDK for Next.js into our project (if you need help with that step, check out my chapter [Sentry.io for Next.js projects](/web_development/posts/sentry-io#sentryio-for-nextjs-projects) in the Sentry.io post)

After creating a Sentry.io account and setting up the SDK I recommend also using the Vercel integration for Sentry.io as this will simply the part **Adding the Sentry authentication token as an environment variable to your CI setup** that we just saw when using the Sentry.io wizard
