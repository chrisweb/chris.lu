...
```shell
npm i @next/mdx @types/mdx @mdx-js/loader --save-exact
```
...
Because later in this tutorial we will add several plugins, what we will do for now is turn off the experimental **mdxRs** by setting the configuration option to **false**, but also add a comment as to why, so that in the future as soon as support for plugins gets shipped we just need to turn it on:
