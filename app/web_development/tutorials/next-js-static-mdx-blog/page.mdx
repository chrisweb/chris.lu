...
First lets open our playground `pages.mdx` we created earlier in the `/app/tutorial_examples/mdx-components_playground` folder and add a codeblock to it:

````md title="/app/tutorial_examples/mdx-components_playground/page.mdx" showLineNumbers {16-22}
<article>
...
Line 16 to 22 we add a code block example to our playground

If the dev server is not already running first start the dev server (using the `npm run dev` command) and then open the [http://localhost:3000/tutorial_examples/mdx-components_playground](http://localhost:3000/tutorial_examples/mdx-components_playground) playground page URL in your browser to have a look at the result:
...
````md title="/app/tutorial_examples/mdx-components_playground/page.mdx" showLineNumbers{16} /showLineNumbers/#special
```js showLineNumbers
function helloWorld() {
    // this is a comment
    let greeting = 'Hello World!'
...
Line 16 we add the `showLineNumbers` keyword to code block markdown
...
````md title="/app/tutorial_examples/mdx-components_playground/page.mdx" showLineNumbers{16} /showLineNumbers{10}/#special
...
Line 16 we updated the `showLineNumbers` adding `{10}` to tell the code block that the first line is number 10
...
````md title="/app/tutorial_examples/mdx-components_playground/page.mdx" showLineNumbers{16} /{1}/#special /{3-4}/#special
...
Line 16 we removed the `{10}` curly brackets to make our line numbers start with 1 again, then we added a space and then a number inside of curly brackets `{1}` to highlight the first line and also added `{3-4}` to highlight line 3 to 4
...
````md title="/app/tutorial_examples/mdx-components_playground/page.mdx" showLineNumbers{16} /#errorLine/#special
...
Line 16 we added a new highlight style ID after the curly brackets defining which line(s) should get highlighted and we set the highlight line ID to `errorLine`

If you launch the dev server, then open the playground URL [http://localhost:3000/tutorial_examples/mdx-components_playground](http://localhost:3000/tutorial_examples/mdx-components_playground) in your browser, right click on the code block and then select **Inspect**, you will notice that because we added the `#errorLine` ID we now have a `data-highlighted-line-id="errorLine"` attribute on the highlight line span, we can now target `data-highlighted-line-id` attributes that have the value `errorLine` in our CSS to change their background (and left line) color(s)
...
````md title="/app/tutorial_examples/mdx-components_playground/page.mdx" showLineNumbers{16} /helloWorld()/1#special
...
Line 16 we removed the highlight line curly brackets from the previous example and instead used two slashs (`/`) to tell rehype pretty code that we want to highlight `helloWorld()`
...
````md title="/app/tutorial_examples/mdx-components_playground/page.mdx" showLineNumbers{16} /#mySpecialHighlight/#special
...
````md title="/app/tutorial_examples/mdx-components_playground/page.mdx" showLineNumbers{16} /2/#special
...
````md title="/app/tutorial_examples/mdx-components_playground/page.mdx" showLineNumbers{16} /tsx/#special
...
````md title="/app/tutorial_examples/mdx-components_playground/page.mdx" showLineNumbers{16}
```
function helloWorld() {
    // this is a comment
    let greeting = 'Hello World!'
...
Line 16 we removed the language flag (and any other options)
...
````md title="/app/tutorial_examples/mdx-components_playground/page.mdx" showLineNumbers{16} {9}
```js
function helloWorld() {
    // this is a comment
    let greeting = 'Hello World!'
...
Next we add code block and a list of inline code examples to our playground page to test our tokens map:

````md title="/app/tutorial_examples/mdx-components_playground/page.mdx" showLineNumbers{26}
```tsx
function helloWorld() {
    // read me
...
Now that the plugin is installed lets add some "GitHub Flavored Markdown" (GFM) examples to our MDX playground page:
...
The footnotes are a bit more complex to use than the other remark-gfm features which is why I decided to create a seperate chapter just for them
