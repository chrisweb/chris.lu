...
Have a look at the first line, remember last time when we used the anchor we had a page request on the first line loading the entire Blog document, but this time we just have a fetch request getting some RSC (react server component) data from the server

As you can see the difference is huge when using the next/link component compared to using a regular anchor element, 
