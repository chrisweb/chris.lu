...
```css title="/app/global.css" showLineNumbers{65} {4-5}
body {
    /* remove any margin that browsers add to body */
    margin: 0;
...
### adding a CSS class lists using mdx-components

First lets create a new playground to experiment with the **mdx-components** file
...
Now open the `mdx-components.tsx` (which is in the root of your project) and add an entry for each headline, like so:
...
Try it out for yourself, ensure the dev server is running and then in the browser navigate to [http://localhost:3000/tutorial_examples/mdx-components_playground](http://localhost:3000/tutorial_examples/mdx-components_playground), then use your browser developper tools inspect tool and you will see that the `<ul>` element now has a **class** attribute containing the value `listContainer`
...
When building our navigation earlier we saw why it is beneficial to use next/link instead of regular HTML anchors, so now we want to do the same for all the markdown links that we will insert into our MDX pages, because without using the **mdx-components** the parser would just transform the markdown links into regular HTML anchors
...
For this component we import [next/link](https://nextjs.org/docs/app/api-reference/components/link) and two types to help us strictly type our variables

First we create an interface for the props of the component by extending the react type **PropsWithChildren**

Then we create a **isExternalUrl** function that gets the URL for our link as input and then checks if the URL is internal or external, if the URL is external it returns true else false, I used a quite naive approach to make the test but I didn't need more for my own URLs (if this is not complex enough for your own needs then feel free to improve that part, for example if you have more than one domain then you may want to use an array instead), for my use cases I just check if a URL starts with a slash or # hashtag which indicates it is an internal URL and I check if a URL starts with http or https in which case it is an external URL, except if the domain matches a value I pass as parameter which is the domain name the app will use in production

Next we add the code for the component itself, first we use the interface we created earlier to strictly type the props, this is useful when you intend to re-use the component somewhere other than in the mdx-components file and want to pass props manually to the component

The core code is pretty self explanatory, we first use the **isExternalUrl** function to check if the URL is an external or internal URL, the first paramater is the URL of the current link and the second parameter is the domain of our project (replace example.com with the domain you intend to use in production), then for (external domains) we set the values for two attributes rel and target

Finally in the return statement we check if the link is external in which case we use a HTML anchor element and add the **external icon** behind it and if it is an internal link then we use next/link
...
Next open the `mdx-components.tsx` (which is in the root of your project) again and and lets add a second markdown component to customize the anchor elements, like so:
...
```css title="/app/global.css" showLineNumbers{120} {1-8}
.iconSmall {
...
#### examples in the MDX playground page
...
launch dev server... [http://localhost:3001/tutorial_examples/mdx-components_playground](http://localhost:3001/tutorial_examples/mdx-components_playground)

congratulations...

### optimizing images in MDX using next/link

can we use next/image to make sure that all images we put into the markdown of our MDX pages use next/image so that they are optimized too

our second goal is to not lose the VSCode feature that allows us to just drag an drop images into a document
