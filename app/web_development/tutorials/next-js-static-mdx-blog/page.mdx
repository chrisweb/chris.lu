...
```js title="next.config.mjs" showLineNumbers{16} {19-22}
    /** @type {import('next').NextConfig} */
    const nextConfigOptions = {
        reactStrictMode: true,
        poweredByHeader: false,
...
Line 34 to 37 we add a new entry for images to our configuration and set the format property to `['image/avif', 'image/webp']`
...
#### Image priority property to optimize LCP
...
There are two important informations in here, the first one is the **Rendered size** which is 1056 pixel (if you have a high density screen this value will be higher), this corresponds to the size the image gets displayed on screen and the second important information is the **Instrinsic size** which is the original size of the image, as you can see in this example the rendered size is half the original size, this shows us that we are fetching an image that is not only two times wider but also two times higher than what we need, meaning the image is 4 times bigger than what we need and this is going to lead to a bad LCP score
...
As we saw in the previous chapter, we told the browser to make the displayed size of our image smaller but we are still loading an image that is larger than what we need, however not every screen has the same size, which means we will have to get different versions of our image, for example bigger versions that get displayed on desktop computers and smaller versions that get used for mobile devices

But wait there is more, today our users don't just have screens of different sizes but also screens with different pixel densities, for example some high density screens can have a resolution of 2 device pixels per CSS pixel (2x), which means that for those high density screens we will need an image that is twice the size of an image we use for a lower density screen
...
In our global.css we did set a maximum width for the `main` element to 70rem, to convert REMs to pixels you need to know the root font size, in our case it is 16 pixel (the default browser font size), which means the max-width of the container in which our image gets displayed is 70 x 16 = 1120 pixels



TODO: explain that if we have an image with an intrinsic size that is smaller than the width of the container it will not get stretched to 100% but get displayed as large as it original width is, that is because the width attribute on the image is what matters most
