...
First we will add the `@next/mdx` package, two dependencies from the `@mdx-js` repository and the MDX types to our next.js project, this package will add support for MDX files to our next.js project, to install the package execute the following command in your VSCode terminal:

```shell
npm install @next/mdx @mdx-js/loader @mdx-js/react @types/mdx --save-exact
```
...
```mjs title="next.config.mjs" {4} {17}
const nextConfig = (phase) => {
