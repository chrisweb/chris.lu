...
To help us achieve that goal we are going to use the [Intersection Observer API](https://developer.mozilla.org/en-US/docs/Web/API/Intersection_Observer_API), as you can see on [caniuse the Intersection Observer API](https://caniuse.com/mdn-api_intersectionobserver) is well supported (expect for IE 11)

An easy way to use the **Intersection Observer API** is to create a React **hook**, so first we create a new `/hooks` folder in the root of our project (or inside the /src folder if this is how you configured Next.js)

Then inside of our `/hooks` folder we create a new `useIntersectionObserver.ts` hook file and add the following content:

```ts title="/hooks/useIntersectionObserver.ts" showLineNumbers

```

Next we need to create a component that will use our **useIntersectionObserver** hook

In the `/components` folder create a new `toc` folder and then in that folder create a new `Hihglight.tsx` component file:

```tsx title="/components/toc/Hihglight.tsx" showLineNumbers

```

When we configured the TOC plugin in the Next.js configuration file, we used it's options object to tell the plugin that we wanted to have an `id` attribute with the value `` on our TOC container, meaning that the `aside` element that acts as container will have ``, we are going to use that id to traget the aside container in our mdx-components.tsx file and then :

```ts title="mdx-components.tsx" showLineNumbers
