...
Line 23 to 32 we use the **html** [type selector](https://developer.mozilla.org/en-US/docs/Web/CSS/Type_selectors) and do several things:
...
* then we also set the [text-size-adjust](https://developer.mozilla.org/en-US/docs/Web/CSS/text-size-adjust)
* we also use the [tab-size](https://developer.mozilla.org/en-US/docs/Web/CSS/tab-size)
* then we set the color for our background and the default color for our text
* finally we set the font-size to 100%, which is good for accessibility as it lets the user change the font-size via the browser settings, as are using the [CSS rem unit](https://developer.mozilla.org/en-US/docs/Learn/CSS/Building_blocks/Values_and_units) for all font related sizes, this means that setting the font-size of the HTML element will have an impact on all other font-sizes in our CSS, for example we use rem for the 

next we use the **body** and first set the default font-size using the css variable we just set in the root pseudo class, then we also reset the mardin values that browser might set by default
...
To experiment with images we first need an image, I created a test image using the funny [GitHub **octocat** image generator](https://myoctocat.com/), to create your own octocat image visit [myoctocat.com](https://myoctocat.com/), then create your own octocat and finally click on the **share** button to download the image, another option is to use a royalty free photo from a website like [pexels.com](https://www.pexels.com/creative-commons-images/) and download one you like
...
<Image src="/images/playground_test.png" />
...
This is the eslint plugin jsx-a11y telling us that it is recommended to always add an alt text to images, not only is this a best practice for accessibility reasons but it can also improve your SEO

So let's first that by adding an alt text, like this:

```mdx title="/app/tutorial_examples/mdx-components_playground/page.tsx" showLineNumbers{16} /alt="purple octocat wearing a diadem, a teal dress, orange bunny slippers and holding a gaming console pad"/#special
<Image src="/images/playground_test.png" alt="purple octocat wearing a diadem, a teal dress, orange bunny slippers and holding a gaming console pad" />
```
...
<Image src="/images/playground_test.png" alt="purple octocat wearing a diadem, a teal dress, orange bunny slippers and holding a gaming console pad" width="200" height="200" />
```

> [!NOTE]  
> I set the width and height to 200 pixels as I used the octocat image I mentioned at the beginning of this chapter, if you are using another image you that is not square then you might want to look at the details of image by hovering over the image in your operating system or by using an image editor like [Gimp](https://www.gimp.org/) and then depending on the ratio you might want to set other values for the width and height
...
The reason why the title of this chapter says "How to NOT add images", is because even though it works, there are actually several problems with this solution, to understand the first one, first open the **browser dev tools** (by pressing the `F12` key or right click somewhere in the page and chose **Inspect**), then open the [Network Tab](https://developer.chrome.com/docs/devtools/network) and then click on the row for the image, then click on **Headers** and look at the entries for the **response headers** where you should have an entry for **Cache-Control**, like in this screenshot:
...
There is something else about this solution that is not great, by using react component (next/image component) we get a bad DX as we need to add an next/image import to all our MDX documents, we also need to write the code to use next/image ourself and finally we need to set the path to the image manually, which is bad because if we have a typo in the path then we will get a 404 instead of an image, in the next chapter we will see a better solution which uses the VSCode drag and drop feature to insert images into a document

Finally there is yet another thing that makes our DX worse, which is that we had to set the width and height of our image manually, in the next chapter we will see a solution so that those values get determined automatically
...
VSCode makes it easy to add images as markdown elements into MDX documents because it has a feature that lets you **drag & drop** images into a MDX document, it then creates the markdown element for you, sets the path to the image and if you hover over the path it shows you a preview of the image

We go back to our `page.mdx` file (that is in the `/app/tutorial_examples/mdx-components_playground` folder) in VSCode and first remove the next/image import as well as the image component itself, we will not need them anymore
...
```md title="/app/tutorial_examples/mdx-components_playground/page.mdx" showLineNumbers {16}#special
<article>
...
Line 16 is the image markdown element that VSCode created after we drag & dropped the image into the page

If you now look at the image markdown element that VSCode has created, you will notice that there is a **drop options** icon at the end, like this:
...
We then replace the **alt text** placeholder with the alt text we already used in the previous chapter:

```md
![purple octocat wearing a diadem, a teal dress, orange bunny slippers and holding a gaming console pad](../../../public/images/playground_test.png)
```

And if you want to have a title you can add it like so:

```md
![purple octocat wearing a diadem, a teal dress, orange bunny slippers and holding a gaming console pad](../../../public/images/playground_test.png 'this is the image title')
```

> [!MORE]  
> [VSCode "Markdown editing" documentation](https://code.visualstudio.com/docs/languages/markdown#_inserting-images-and-links-to-files)  

#### Automatic image sizing using image imports to optimize CLS

As we saw in the previous chapter one of things next/image helps you get right is [image sizing](https://nextjs.org/docs/app/building-your-application/optimizing/images#image-sizing), meaning it needs to know the width and height of an image to be able to reserve the space the image will use when it has finished loading
...
next/image offers you 3 options to tell it what the display size will be:
...
In the first example we did set the width and height manually, of course if you have to manage lots of images this is just bad for the DX, so what we want is to have next/image detect the width and height automatically, next/image can do this if we use static imports, but if we would add the static imports manually this would again be bad for our DX, so this is why (in the next chapter) we will install and configure a rehype plugin that will scan our MDX documents and automatically convert all image paths it finds to static imports and then next/image will be able to detect the width and height of our images
...
#### Rehype plugin to convert all image paths to static imports

This chapter consists of 4 steps, first we will add the rehype plugin package to our project, which will convert the path of our markdown images into an import, next we will update our Next.js configuration file to use the rehype plugin to our MDX setup, then we will create a simple image component that will use next/image so that our images get optimized and finally we will add our image component to the mdx-components file to make sure every markdown element uses our image plugin
...
Line 3 we import the **remark-mdx-images** plugin

Line 12 we add the **rehypeMDXImportMedia** plugin to the **rehypePlugins** list of our MDX configuration

Next we are going to create a simple **BaseImage** component, similar to what we did earlier for the links, go into the `/components/base` folder we created earlier, then create a new `Image.tsx` file and add the following code:
...
In the first chapter when we launched the dev server we got an error from Next.js telling us to set the image height and width manually, this time we don't have such an error because we now use the rehype plugin to convert the image path to an import and this allows next/image to automatically detect the width and height

Another problem we saw in first chapter was that our image response had a **Cache-Control header** with the value set to **public, max-age=0, must-revalidate**, now if we open the **browser dev tools** again (by pressing the `F12` key or right click somewhere in the page and chose **Inspect**), then open the [Network Tab](https://developer.chrome.com/docs/devtools/network) then click on the row for the image, then click on **Headers** and look at the entries for the **response headers** we can see that the **Cache-Control** value changed:
...
As we can see the max-age now has a very high value and is not zero anymore and the [immutable directive](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Cache-Control) got added too, telling the browser that this resource will not change, which is another benefit we get from using static import

Congratulations you just created a complete workflow where every image you drag & drop into your MDX documents will have its path automazically converted to an import so that next/image can determine the width and height (without you having to set it manually) and you created a custom react image component that will automatically optimize every markdown image 🎉
...
Line 4 to 31 we add a **extractImageType** function that will extract the image "type" information that we put in our image titles, a type information is a keyword inside of curly braquets that lets us apply different options to groups of images, after extracting the type the function will remove the braquets and their content from the title and put what is left back into the type attribute
...
![purple octocat wearing a diadem, a teal dress, orange bunny slippers and holding a gaming console pad](../../../public/images/playground_test.png 'this is the image title { banner }')

</article>

```

Line 14 we added the `{ banner }`#special keyword (inside curly braquets) to our image, so that our **BaseImage** component knows that it should add the **priority** property
...
There are two important informations in here, the first one is the **Rendered size** which is 1056 pixel (if you have a high density screen this value will be higher), this corresponds to the size the image gets displayed on screen and the second important information is the **Intrinsic size** which is the original size of the image, as you can see in this example the rendered size is NOT the same as the original size, this difference in sizes shows us that we are fetching an image that is not only two times wider but also two times taller than what we need, this is obviously going to lead to a bad LCP score because the heaviest item on the page is our image and it is 4 times heavier than it should be
            ...
            {imageType === 'banner' ? (
                <>
                    ...
                    <Image
                        ...
                        sizes="(max-width: 70rem) 100vw, 1056px"
                        {...newImageProps}
                    />
                </>
            ) : (
...
Line 50 we add the `sizes` attribute and added the `(max-width: 70rem) 100vw, 1056px` media query as value

Our sizes attribute will tell the browser that for a maximum viewport width of 70rem it should chose an image to display that is equal or bigger than 100vw (100vw means 100% of the view width) and for any other viewport size above 70rem the browser should always take an image that is 1056 CSS pixels wide

You might wonder why 1056, well because in our specific use case we have defined in our global.css that the maximum width for the `main` element to 70rem, to convert REMs to pixels you need to know the root font size, in our case it is 16 pixel (the default browser font size), which means the max-width of the `main` element is 70 x 16 = 1120 pixels, in `main` element we have an `article` element which has a padding left and right of 2rem (so 2 x 16 = 32 pixels), this means we need to substract that padding from the `main` element width to get the maximum width an image can have, so 1120 - (2 x 32) = 1056 pixel
...
`image?url=%2F_next%2Fstatic%2Fmedia%2Fplayground_test.50052472.png&w=1080&q=75`

and on a screen with a **device pixel ratio** of **2** the URL will instead be this:

`image?url=%2F_next%2Fstatic%2Fmedia%2Fplayground_test.50052472.png&w=3840&q=75`

As we can see, depending on the **device pixel ratio** the browser will fetch a different source, for a **device pixel ratio** of **1** the URL has a **w** parameter with the value **1080**, but when the **device pixel ratio** is **2** the **w** parameter has a value set to **3840**
...
>  
> In this tutorial we deploy to vercel, in preview and production the vercel service will do the image resizes (in the cloud) for us  
>  
...
```html
<img
    alt="purple octocat wearing a diadem, a teal dress, orange bunny slippers and holding a gaming console pad"
    ...
    sizes="(max-width: 70rem) 100vw, 1056px"
    srcset="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fplayground_test.50052472.png&amp;w=640&amp;q=75 640w, /_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fplayground_test.50052472.png&amp;w=750&amp;q=75 750w, /_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fplayground_test.50052472.png&amp;w=828&amp;q=75 828w, /_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fplayground_test.50052472.png&amp;w=1080&amp;q=75 1080w, /_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fplayground_test.50052472.png&amp;w=1200&amp;q=75 1200w, /_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fplayground_test.50052472.png&amp;w=1920&amp;q=75 1920w, /_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fplayground_test.50052472.png&amp;w=2048&amp;q=75 2048w, /_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fplayground_test.50052472.png&amp;w=3840&amp;q=75 3840w"
    src="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fplayground_test.50052472.png&amp;w=3840&amp;q=75">
```
...
The value of the **srcset** contains a series of image **URLs** followed by a **number** and that number has the letter **w** appended, for example the first entry `/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fplayground_test.50052472.png&amp;w=640&amp;q=75 640w` in the list, is a string which contains the image source URL, then a space and then something called a **condition descriptor**, in this case the condition descriptor that tells the browser that if it needs an image with a width of 640 pixel (640w) then it should take the image source URL that preceeds it

If we now put everything together here is what the browser does for a screen with a regular density:

* first the browser determines at what CSS **image width** we want to display the image, it knows that the **style** attribute has set the image width to 100% of the container, the container without the padding is 1056 pixels wide, so the first information is that it will need an image that is **1056 pixels wide**
...
* so now the browser does the math, image CSS width x device pixel ratio = image source width, so 1056 x 1 = 1056 pixels, at this point it now knows it will need an image source file that is at least 1056 pixels wide
* next the browser looks at what image souces we offer by looking at the **srcset** sources list, it attempts to find an URL for an image width that is equal or bigger than the 1056 pixels image it needs and finds an entry with a **condition descriptor** that is **1080w**, so it will use the image URL `/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fplayground_test.50052472.png&amp;w=1080&amp;q=75`

For a screen with a higher density the steps are the same but the result is not, this is because this time the **device pixel ratio** is **2**, so it does the math again and finds out it needs an image source that is 1056 x **2** = 1056 pixels wide, so it looks at the srcset again and the image source URL that is equal or bigger than 1056 pixels is `/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fplayground_test.50052472.png&amp;w=3840&amp;q=75`, which is the URL for the source with a width of 3840 pixels
...
Now that we know how sizes and srcset works, there is one last thing we want to improve, we saw that for screens with a 2x device pixel ratio the browser will chose the source URL that will fetch a 3840 pixels wide image, the problem here is that this 3840 pixels wide image will be almost twice as large as the 2112 pixels wide image that we need, the browser can NOT take a smaller image because the second biggest version we offer is 2048 pixels and that is slightly lower than the 2112 pixels wide image we need
...
There are two values in the list that are very close to each other, 1920 and 2048, that 2048 one is just a bit too small for our need but also very close to 1920, so it seems like a good idea to replace that value with the one we need which is 2112
...
const nextConfig = (phase) => {
    ...
    const nextConfigOptions = {
		...
		images: {
            // file formats for next/image
            formats: ['image/avif', 'image/webp'],
            deviceSizes: [384, 640, 750, 828, 1080, 1200, 1920, 2112, 3840],
        },
    }
...
Line 36 we add the **deviceSizes** option and set the value to `[384, 640, 750, 828, 1080, 1200, 1920, 2112, 3840]`

If we now open the browser (assuming you are on a device with a high density screen) and look at the `img` element we will see that srcset now has an entry for 2112 pixels wide images and if we open the developer tools and look at the image URL in the Network tab we can see that the browser has fetched the image using the source URL for a 2112 pixels wide image
...
```js title="next.config.mjs" showLineNumbers {4} {13}
import { PHASE_DEVELOPMENT_SERVER } from 'next/constants.js'
import createMdx from '@next/mdx'
import rehypeMDXImportMedia from 'rehype-mdx-import-media'
...
```js title="next.config.mjs" showLineNumbers {8} {9-11} /[rehypePrettyCode, rehypePrettyCodeOptions]/#special
import { PHASE_DEVELOPMENT_SERVER } from 'next/constants.js'
import createMdx from '@next/mdx'
import rehypeMDXImportMedia from 'rehype-mdx-import-media'
import rehypePrettyCode from 'rehype-pretty-code'
...
What we do here is nothing fancy, we just reset the margin and padding on the figure element that is the container of a **rehype-pretty-code** code block, it has a **data** attribute that we use to target it as well as the **pre** and **code** element inside of it

Then we add a margin to the figure element 
...
* check out what [shiki transformers](https://shiki.style/guide/transformers#transformers) are, can they be useful for our blog, [npmjs "shiki transformers" package](https://www.npmjs.com/package/@shikijs/transformers)
...
TODO: if code block titles for filenames are not supported out of the box by **rehype pretty code** then add this [npmjs rehype-code-titles package](https://www.npmjs.com/package/rehype-code-titles)
