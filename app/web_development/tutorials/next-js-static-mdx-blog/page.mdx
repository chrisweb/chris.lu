...
For the third and last part we are going to use [next/image](https://nextjs.org/docs/app/api-reference/components/image) to make sure that all images we put into the markdown of our MDX pages get optimized

Our second goal is to make sure we do NOT lose the VSCode feature which allows us to just drag an drop images into a MDX document and have VSCode create the markdown image element
...
```tsx title="/components/base/Link.tsx" showLineNumbers {11-13}#special /props.alt.startsWith('banner')/#special
...
This component is fairly simple, it is mostly a wrapper using next/image but with two small features, the first feature is line 11-13 where we check if the image is a gif, if the image is a gif then we don't use the next/image placeholder (blur) feature as animated images are not supported, if you don't exclude animated images like Gifs you will get an error that no [blurDataURL](https://nextjs.org/docs/messages/placeholder-blur-data-url) is present
