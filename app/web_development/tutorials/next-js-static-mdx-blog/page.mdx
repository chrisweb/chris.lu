...
To solve this you could write custom react component that you import into the MDX document an that you use instead of the default markdown syntax, or another solution would be to find a remark or rehype plugin that solves that problem for us, but both solutions would require more work (compared to using the mdx-components file) and also by using the mdx-components file allows us to customize the output without altering the markdown syntax, meaning our content is still compatible with other markdown parsers
...
```tsx title="/components/base/Link.tsx" showLineNumbers
