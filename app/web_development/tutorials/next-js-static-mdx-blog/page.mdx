...
Now that the plugin is ready lets make a test, for this we will create a new MDX page
...
We have added some markdown headings of different levels to our MDX page, they are wrapped inside of an `<article>` element (which is optional but I like to use semantic elements when ever it makes sense), the extra line break elements are so that we can better see how the sticky position of the aside element works when we add the CSS in the next chapter

After the `<article>` element we have inserted the placeholder for our **table of contents (TOC)**

Finally make sure the dev server is running and then visit the toc playground page in your browser and you will see that the plugin has created a **table of contents** for us, it has included the first the 3 levels of headings but excluded the level 4 heading as this is what we specified in the configuration by setting the maximum depth option to 3, if you inspect the HTML code you will notice that it also added the **id** attribute to `<aside>` container as well as the **aria label** to the inner `<nav>` element
...
#### highlight the link to the current headding
