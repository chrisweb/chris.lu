...
If you now look at the image markdown element that VSCode has created, you will notice that there is a **drop options** icon, like this:
...
#### Image formats & priority to optimize LCP
...
> [!WARN]  
> The solution I'm about to show you is clearly a hack, it works well in my blog [chris.lu](https://chris.lu) for the usage I have but might not work well for you depending on what your use case is, so you might want to find another solution that better suits your needs, if you want something more "elaborate" maybe the [remark-directive](https://github.com/remarkjs/remark-directive) plugin is what you need
>
> what I like about my solution is that it does require yet another plugin that would also require that we use a modified markdown image syntax

To add the priority property we first need to update our **BaseImage** component to this:

```tsx title="/components/base/Image.tsx" showLineNumbers {12}
import Image from 'next/image'
import type { ImageProps } from 'next/image'
...
Line 12 we add the next/image **priority** property
