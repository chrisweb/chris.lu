...
```js title="next.config.mjs" showLineNumbers {7-8} {14-17} {46}#special {61}#special
// CSP headers here is set based on Next.js recommendations:
// https://nextjs.org/docs/app/building-your-application/configuring/content-security-policy
const cspReportOnly = true;
...
Line 82 to 92 we added a new variable `extraSecurityHeaders` to store the HSTS header as we want to exclude it in development where we don't have an SSL certificate
...
Line 110 we use the `extraSecurityHeaders` to add the `Strict-Transport-Security` header to the list of headers
