...
Line 64 to 70 same as for the `<main>` element we again set the **display** property to **flex** and the **flex-direction** to column to make sure all elements inside of our `<article>` element get displayed one under the other, then we add a padding using our `--spacing` CSS variable to 16 pixels on each side, because we also added maximum width to the main element this means that article content (if we don't have an aside) will be a maximum 1120 - 32 = 1088 pixels wide, this will matter in a later chapter when we start adding images to our cotnent, finally we set the width to 100% to make sure that the article always uses the full width of the parent main element

Line 72 to 74 we use our default spacing variable to add some margin to our paragraphs

Line 76 to 82 we add some color to all our links using the CSS `var()` function to use the variables we set earlier in the  and we also use the [:hover](https://developer.mozilla.org/en-US/docs/Web/CSS/:hover) pseudo class to change the color of links on hover

Line 84 to 112 we use the font-size and line-height properties to set a custom height for our headings (like any other values in this CSS stylesheet, those are just my examples, feel free to tweak those values to your liking)
...
```css title="/app/global.css" showLineNumbers{116} {1-8}
.iconSmall {
...
Line 116 to 123 we add some CSS to style our icon:
...
```css title="/app/global.css" showLineNumbers{125} {1-5} {7-11}
...
```css title="/app/global.css" showLineNumbers{125} {11}
[data-rehype-pretty-code-figure],
[data-rehype-pretty-code-figure]>pre {
    margin: 0;
    padding: 0;
...
Line 135 we add our `background-color`, I chose a very dark color to make we have a high contrast between the code colors and the background
...
```css title="/app/global.css" showLineNumbers{138}
...
```css title="/app/global.css" showLineNumbers{163} {2-6} {8-11}
...
Line 164 to 168 we use a selector to target all lines, lines have a `data-line` attribute, the CSS we add will border on the left which will create a vertical line that we will then be able to colorize when the line is highlighted but by default we set it to `transparent`, to create this vertical line is of course not mandatory, if you don't like it just leave it away

Line 170 to 173 we use a selector to target all line that have a `data-highlighted-line` attribute, we change the background color of the line as well as the color of the new border (vertical line) we added on the left
...
```css title="/app/global.css" showLineNumbers{175}
...
Line 175 we added a selector to target the `data-highlighted-line-id` attribute if it has the value `errorLine` and on the next we added a more reddish color for the background and vertical line to make this line appear like a line with an error
...
```css title="/app/global.css" showLineNumbers{180}
[data-highlighted-chars] {
    background-color: #432936;
}
```

Line 180 we add a custom background color for highlighted characters
...
```css title="/app/global.css" showLineNumbers{184}
...
```css title="/app/global.css" showLineNumbers{188}
...
We have added some markdown headings of different levels to our MDX page, they are wrapped inside of an `<article>` element (which is optional but I like to use semantic elements when ever it makes sense)
