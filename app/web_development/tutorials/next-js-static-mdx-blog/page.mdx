...
If we tell rehype pretty code what programming language we used for the code inside of the code blocks then the code block highlighting colors will get adjusted, so far we always used javascript by adding the language hint `js`, if you want to use **typescript** instead then you can switch the language hint that is behind the first backticks to `tsx`, like so:
