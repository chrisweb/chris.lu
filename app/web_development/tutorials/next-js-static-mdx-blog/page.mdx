...
Next we are going to use the [pathname](https://developer.mozilla.org/en-US/docs/Web/API/URL/pathname) of the current page that we can get using **next/navigation** to check if the first segment of the current path matches one of the entries in our navigation
...
### making the navigation fancy using next/font

Now that our Navigation is working we will add a few touches of color and use next/font to add an custom font

The advantage when using next/font is that it will reduce layout shifts because the text initially uses a default font until the custom font has loaded
