...
Line 16 we use the **BaseImage** component, the path of our image is `/public/images/playground_test.png`, but we set the `src` to `/images/playground_test.png` (we omit the `/images` folder) as Next.js will assume the files are in the public folder
...
So far so good, the image gets displayed, but if you now click
