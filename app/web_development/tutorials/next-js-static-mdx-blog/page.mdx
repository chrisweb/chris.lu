...
```css title="/app/global.css" showLineNumbers{66} {4-5}
body {
    /* remove any margin that browsers add to body */
    margin: 0;
...
Or for example in the previous chapter we replaced anchor elements in our navigation component with next/link, how do we now replace markdown links (that will be turned into HTML anchors) with next/link, or how can we use next/image to make sure that all images we put into the markdown of our MDX pages use next/image so that they are optimized too

For those 3 examples you could write custom react component that you import into the MDX document an that you use instead of the default markdown syntax, or another solution would be to find a remark or rehype plugin that solves our problem, but both solutions would require more work (compared to using the mdx-components file) and would reduce the compatibility of our markdown content, for example if at some point we decide to use it in a product that has a markdown parser that does not have those components or plugins installed then that parser could not cope with the extra syntax we added to our markdown or for example if we use the VSCode feature to drop an image into a markdown or MDX document and VSCode automatically creates the markdown image sysntax for you would be lost if we use a react component or a special syntax that is required for a plugin to work

This is why using the **mdx-components** file has the advantage that it lets us keep using default markdown syntax while still enhancing the (HTML) output that we get
