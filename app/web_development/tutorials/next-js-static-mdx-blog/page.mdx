...
Instead of using **Next lint** CLI command for linting we will create our own command using the **ESLint CLI**

Change the **lint script** in the  to this:
...
```mjs title="package.json" {4-5}#special
{
    "scripts": {
        "lint": "npx eslint ./ --ext .js,.jsx,.ts,.tsx,.mdx,.md --cache --cache-location .next/cache/eslint",
        "lint-nocache": "npx eslint ./ --ext .js,.jsx,.ts,.tsx,.mdx,.md",
...
The **lint-debug** script is again the same linting command but this time we add the `--debug` flag, this will print a lot of useful information about what ESLint does in the **OUTPUT** tab in VSCode, which is again useful if you tweak your configuration and want to verify the things ESLint is doing or to debug a problem with your setup, like checking if your plugins get loaded correctly and much more
...
When we now use our `npm run build` command (or when our deployment script uses it), it will first execute `npm run lint` and if there are no linting errors it will then execute `next build`
