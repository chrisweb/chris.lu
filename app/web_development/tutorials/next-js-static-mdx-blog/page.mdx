...
> [Next.js "image caching behavior" documentation](https://nextjs.org/docs/pages/api-reference/components/image#caching-behavior)  
...
> [!MORE]  
> [Next.js "image component" reference](https://nextjs.org/docs/pages/api-reference/components/image)  
...
This is a warning from **next/image** telling us that it has detected that our image is above the fold, meaning the image is inside the first part of our HTML document that is visible when the page loads and because of that it is recommended to use the [priority](https://nextjs.org/docs/pages/api-reference/components/image#priority) property
...
But wait if we now add the **priority** property to next/image then our **BaseImage** component will set the property to all images no matter if they are on top of the page or further down, which is why we need a way to distinguish between images that should be high priority and images that should be low priority
...
```tsx title="/components/base/Image.tsx" showLineNumbers {4-33} {39}
import Image from 'next/image'
import type { ImageProps } from 'next/image'

function updateProps(imageProps: ImageProps) {
...
Line 4 to 33 we add a function called **updateProps** that will extract some information that we call **imageType** that we add at the end the title inside of curly braquets and then removes the braquets and their content from the title, we then use the  **imageType** to add extra props to the image, if the imageType equals **banner** then we set the **priority** prop

Line 39 we use our new **updateProps** function

What we did here is add a new feature that allows us to specify an **image type** for each image, for now we only check if the type is **banner** and if it is we set the **priority** property on the image, we can later extend this feature to check for other types, like for example if an image is of type **header** we could decide to set the **fill** property as well as some CSS, how you name the types and what you do based on a type is up to you

Now all we need to do is update the title of our markdown image element and use our new feature, like so:

```md title="/app/tutorial_examples/mdx-components_playground/page.mdx" showLineNumbers {16} /{ banner }/#special
<article>
...
We added the `{ banner }`#special information to our image title to tell our **BaseImage** component that this image needs the **priority** property

If you now go back into the browser and hover over the image you should see the title appear but with the `{ banner }`#special information, if you open the developer tools you should see in the console tab that the warning is gone and if you open the elements tab (or right click on the image and then select inspect) you should see that image now has an the attribute **fetchpriority** set to **high**
...
```tsx title="/components/base/Image.tsx" showLineNumbers{35} {11-14}
const BaseImage: React.FC<ImageProps> = (props): JSX.Element => {
...
Line 11 to 14 we set the width to 100% so that the image width matches the width of our container and we set the [height to auto](https://developer.mozilla.org/en-US/docs/Web/CSS/height#auto) to keep the original ratio of the image, hence the browser will decide what the height should be based on the images width

Now go back to the playground in the browser and now the rendered image should be smaller

Next right click on the image and chose **Inspect** to open the developer tools, you will notice that next/image has added used the [srcset attribute](https://developer.mozilla.org/en-US/docs/Web/API/HTMLImageElement/srcset) and set a value that does NOT contain only one source but 2, behind the URL of the image you can see the pixel density descriptor 1x or 2x where 1x is for regular screens and 2x is for high density screens

Next hover over one of the image URLs in the srcset and you the developer tools will show you a modal that looks like this:
...
```tsx title="/components/base/Image.tsx" showLineNumbers{35} {15}
const BaseImage: React.FC<ImageProps> = (props): JSX.Element => {
...
Line 15 we add the `sizes` attribute

For the sizes we have set as value `(max-width: 70rem) 100vw, 1056px`, the meaning of this string is that we tell the browser that for a maximum width of 70rem it should chose an image to display that is equal or bigger than 100vw (100vw means 100% of the view width) and for any other screen size above 70rem the browser should always take an image that is 1056 css pixels wide
...
The **w** parameter represents the **width** the image should have, the browser automatically fetches an image suitable for the **device pixel ratio** of the screen, as we want an image that is 1056 pixel wide the browser loads an image with **w** set to 1080 pixel for screens with a **device pixel ratio** of **1**, but loads an image where **w** is set to 3840 pixel when the **device pixel ratio** is **2**

This brings up two questions, how does the browser decide what value to use for the **w** parameter when requesting images and secondly why are the sizes the browser choses not equal to the size needed?

The answer to both questions is because of the [srcset attribute](https://developer.mozilla.org/en-US/docs/Web/API/HTMLImageElement/srcset) attribute

When we set the `sizes` attribute next/image will automatically add an `srcset` attribute to the images

We now go back into the browser and right click on the image, then select **Inspect** and if we look at the image elment we see something like this (I formatted mine to make it a bit easier to read):
...
We can see the image element has our **style** attribute we added earlier, as well as the **sizes** attribute we added just now, but below there is also the **srcset** attribute that next/image added

The value of the **srcset** contains a series of image **URLs** followed by a **number** that has the letter **w** appended, for example the first entry `/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fplayground_test.50052472.png&amp;w=640&amp;q=75 640w` in that list of image URLs and something called a **condition descriptor** tells the browser that if it needs an image with a width of 640 pixel (640w) then it should take that URL

If we now put everything together here is what the browser does for a screen with a regular density:

* first the browser determines what CSS **image width** we want, it knows that the **style** attribute has set the image width to 100% of the container which without the padding is 1056 pixel wide
* the browser also knows that our **device pixel ratio** is **1**
* so the browser does the math: image width x pixel ratio = width (1056 x 1 = 1056), so it knows it will need an image that is at least 1056 pixel wide
* next the browser looks at what images we offer to load by looking at the **srcset** value, it attempts to find an URL for an image width that is equal or bigger (but never smaller) than the 1056 pixel image it needs and finds an entry with a **condition descriptor** that is **1080w**, so it will use the image URL `/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fplayground_test.50052472.png&amp;w=1080&amp;q=75`

For a screen with a higher densitsy the steps are the same but the result is not:

* first the browser determines what CSS **image width** we want and it is still 1056 pixels
* however this time the browser notices that the **device pixel ratio** is **2**
* so the browser does the math again: image width x pixel ratio = width (1056 x 2 = 2112), so it knows it will need an image that is at least 2112 pixel wide
* next the browser looks at what images we offer to load by looking at the **srcset** value and finds an entry with a **condition descriptor** that is **3840w**, so it will use the image URL `/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fplayground_test.50052472.png&amp;w=3840&amp;q=75`

There is one last thing we want to improve, we saw that for screens with a 2x device pixel ratio the browser will chose the URL that will fetch a 3840 pixel wide image, the problem here is that this 3840 pixel wide image will be almost twice as large as the 2112 pixel wide image that we need, the browser can NOT take a smaller image because the second biggest version we offer is 2048 pixel and this is lower than the 2112 pixel wide image it needs to display

The solution is to tweak the list of sizes that next/image will use when it creates the srcset and to do so we need to edit the Next.js configuration file of our project

By default Next.js will use the following values for srcsets: 640, 750, 828, 1080, 1200, 1920, 2048, 3840

There are two values in the list that are very close to each other, 1920 and 2048, that 2048 one is just a bit too small for our need so lets replace that value with the one we need which is 2112
...
If we now open the browser and look at the `img` element we will see that srcset now has an entry for 2112 pixel wide images and if we open the developer tools and look at the image URL in the Network tab we can see that the browser has fetched the image using the URL for a 2112 pixel wide image

> [!NOTE]  
> On local the images will get automatically resized and cached by Next.js which uses a Node.js tool called [sharp](https://www.npmjs.com/package/sharp)  
>  
> In this tutorial we deploy to vercel and vercel will do the image resizes for us  
>  
> If however you use another provider you either need to find one that works like vercel or you can use a third party image optimization service like for example [Cloudinary](https://next.cloudinary.dev/) but if you search for "image hosting saas" I'm sure you will find a lot of alternatives, if you use a third party service also make sure you check out the [next/image configuration options](https://nextjs.org/docs/pages/api-reference/components/image#configuration-options) documentation  
> 
> Of course there are also other options like resizing the images at build time and then upload them to a CDN or a build a custom solution on a dedicated server, it all depends how much time you want to spend working on a solution and what your budget is
...
Next we are going to set the **placeholder** property to **blur** which tells Next.js that we want it to create a blurry placeholder for every image and display that placeholder while the image loads, by using a placeholder to fill the space the image will take when it has finished loading we indicate to the user there something will appear soon instead of having the user wonder why there is big empty spot in the page

To set the **placeholder** property we are going to edit our **BaseImage** component and update the code to this:

```tsx title="/components/base/Image.tsx" showLineNumbers{35} {16}
const BaseImage: React.FC<ImageProps> = (props): JSX.Element => {
