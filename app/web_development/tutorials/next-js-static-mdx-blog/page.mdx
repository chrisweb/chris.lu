...
> [!TIP]  
> Statically importing images at build time is important as it allows next/image to automatically determine their width and height, by sizing the images next/image will help prevent **cumulative layout shifts** (CLS) which is a **Core Web Vital metric**, without a width and heigt value the image would get displayed as an empty element until it has has finished loading, then when the image appears it uses some space in the document, meaning other content will get down pushed around and this is what is called a layout shift  
>  
> layout shifts are bad because if they happen at a moment where the user attempts to for example click on something then there is a chance that the user will misclick, this is why making sure make sure that all images reserve the space they will need when finsished loading  

Our main goal is to make sure that adding images to a document is easy and quick by using VSCode to drag and drop images into a document, VSCode will create the markdown element with the path to the image for us, we will also use a remark plugin to make sure the path gets turned into an import
