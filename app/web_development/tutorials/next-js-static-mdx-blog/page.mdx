...
Next.js comes with a typescript plugin, to make it work you need to change the Typescript version that VSCode is using to the **workspace version** (the version that Next.js create app just installed), because by default VSCode will use a built in Typescript version
...
finally I recommend you commit / sync the new `.vscode/settings.json` and the changes you did in the `next.config.mjs`
