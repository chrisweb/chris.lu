...
modified: 2024-10-18T18:18:18.444Z
...
## Changing the Next.js start (production) server port

To manually change the port used by the npm run start command you can do it in the package.json like so:
...
This would always set the port to 3001, but for production you may prefer to use the following command to let your deployment script use a specific value like so:
