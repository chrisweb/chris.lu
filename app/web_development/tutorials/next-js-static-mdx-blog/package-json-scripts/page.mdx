...
Using **npm run scripts** has the advantage of not needing to type (or even remember) the full command but only the alias. It is also great because as the package.json is part of our project, it means that we share those commands, get shared with everyone else who might work on the project with us.
...
This will show you the 4 run-scripts commands added by create next app.
...
As you may have noticed, when we used `npm run dev`, it started the dev server using port 3000. This is because port 3000 is the default port used by Next.js. If port 3000 is already in use, Next.js will use the next best free port, for example, 3001, 3002, ...
