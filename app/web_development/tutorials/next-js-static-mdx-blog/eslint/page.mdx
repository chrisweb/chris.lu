...
TODO: check out the nodejs.org repo, they use next.js, their eslint is very well written: [](https://github.com/nodejs/nodejs.org/blob/main/.eslintrc.json)

# ESLint

Next.js has two different packages related to ESLint, one is called **eslint-config-next** consists of setup for common React ESLint plugins that are useful to lint react code, the other one is called **eslint-plugin-next** and this is an ESLint plugin which adds rules to lint the code of Next.js projects

to learn more about the two packages check out my short introduction in the this [Next.js ESLint packages](/web_development/posts/eslint-next-js) post
...
> [!NOTE]
> spoiler alert I will use option 4 for this project, but feel free to read about the first 3 options so that you know what is possible and then chose for yourself which one you like best

if you want to use one of those 4 options and know more about the differencies between them, then I recommend you first read my seperate post about [installing and configuring ESLint in a Next.js project](/web_development/posts/eslint-next-js)
...
run the following command to install **eslint-plugin-next** and the other plugins manually

`npm i eslint eslint-plugin-react eslint-plugin-react-hooks @next/eslint-plugin-next --save-dev --save-exact`
