...
keywords: ['VSCode', 'ESLint', 'MDX', 'extension', 'linting', 'editor', 'ESLint server']
...
Now we are going to test if VSCode displays error inside of TypeScript code by creating a Button component with 2 errors in it
...
So, as you can see, the linting of TypeScript code is working, too, we got an error from the **@typescript-eslint** and one from the **eslint-plugin-react**
