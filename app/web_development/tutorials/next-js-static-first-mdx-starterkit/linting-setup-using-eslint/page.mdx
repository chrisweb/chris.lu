...
Line F: We use the `files` option to tell ESLint which files (extensions) we want to include (for this tutorial we will enable linting for regular [Typescript](https://www.typescriptlang.org/) (.ts), Typescript and [JSX](https://www.typescriptlang.org/docs/handbook/jsx.html) files (.tsx) and [ES Module (esm)](https://nodejs.org/api/esm.html#modules-ecmascript-modules) files (.mjs))
