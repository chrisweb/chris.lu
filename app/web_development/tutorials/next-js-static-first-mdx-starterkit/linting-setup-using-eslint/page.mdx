...
const tseslintConfig = tseslint.config(
    {
        ...
        languageOptions: {
            parserOptions: {
                ...
                // better keep it turned on, unless you need to
...
Line F: We use the `files` option to tell ESLint which files (extensions) we want to include (for this tutorial we will enable linting for regular typescript (.ts) and typescript react files (.tsx))
...
Lines F to G: Then we add some parser options, those are needed if you use one of the **"type checked"** sets (if are NOT using any `TypeChecked` rules, then you can delete (or uncomment) the `languageOptions` block)
...
### Removing the ESLint and types

After removing **next/typescript** (in a previous chapter) we now also removed **next/core-web-vitals** (as we now add those rules to our ESLint setup using the custom next config we just created).

As both packages that needed a compatibility layer were gone, we also removed the **FlatCompat import** (from the ESLintRC Library) in our code

The last step is to also remove the classic configurations **ESLintRC Library** (as well as the types package) from our dependencies using the following command:

```shell
npm remove @eslint/eslintrc @types/eslint__eslintrc
```

## Adding stylistic rules like 

If you use prettier in your project then you probably don't need the rules we are about to add, the [@stylistic/eslint-plugin](https://www.npmjs.com/package/@stylistic/eslint-plugin) adds rules (and auto fix) that are related to coding style, like double or single quotes, 2 or 4 spaces indentation, semicolons at the end of a line or not, ...

> [!TIP]  
> If you use VSCode and want to quickly format your code (which has a good javascript source code formatting tool) then you can either right click into your document and choose **Format Document** or you can use the shortcut `Shift+Alt+F` (macOS: `⇧⌥F`, Linux: `Ctrl+Shift+I`) to tell VSCode to format your code

The **@stylistic/eslint-plugin** is the recommended plugin, as it contains support for Javascript, Typescript and JSX, it includes different other plugins [@stylistic/eslint-plugin-js](https://www.npmjs.com/package/@stylistic/eslint-plugin-js), [@stylistic/eslint-plugin-ts](https://www.npmjs.com/package/@stylistic/eslint-plugin-ts), [@stylistic/eslint-plugin-jsx](https://www.npmjs.com/package/@stylistic/eslint-plugin-jsx) and [@stylistic/eslint-plugin-plus](https://www.npmjs.com/package/@stylistic/eslint-plugin-plus)

For a list of the **@stylistic rules** check out the [rules section in their documentation](https://eslint.style/packages/default#rules)

First we need to add the plugin to our dev dependencies:

```shell
npm i @stylistic/eslint-plugin@latest --save-dev --save-exact
```
