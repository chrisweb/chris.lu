...
## The state of ESLint v9 and ESLint flat config files in Next.js 15

> [!TIP]  
> If you already have a custom ESLint setup in your project and want to migrate your ESLint configuration file from ESLint classic configuration files to Flat Config files, then have a look at [ESLint configuration migrator](https://eslint.org/docs/latest/use/configure/migration-guide#migrate-your-config-file) as it will automize a big chunk if not all of the migration for you (in some cases with very complex configurations the plugin might fail, in my personal tests it always worked well)  
>  
> Another tip is for cases where you need to use a plugin that has not finished the migration to ESLint v9. You might see errors like this:
> 
> ```shell
> TypeError: context.foo is not a function
> ```
> 
> In that case have a look at the [ESLint Compatibility Utilities](https://eslint.org/blog/2024/05/eslint-compatibility-utilities/)

Next.js 14 was still using ESLint v8, meaning it was using the [ESLint "Classic" configuration files](https://eslint.org/docs/latest/use/configure/configuration-files-deprecated), which was the default for all ESLint versions below 9
...
In a recent [comment (14th Oct. 2024)](https://github.com/vercel/next.js/issues/64409#issuecomment-2411304733) a member of the Next.js team confirmed that Next.js would use ESLint v9 flat configs:
...
I have tracked some of those dependencies myself, here is a small recap if you are interesting in such things:

* The plugin maintainers have started to add to support for ESLint v9 to their projects but not all have released a stable version with ESLint v9 support (as of now oct. 2024).
* For example the plugin [eslint-plugin-react-hooks](https://github.com/facebook/react/releases/tag/eslint-plugin-react-hooks%405.0.0) introduced support for ESLint v9 on Oct. 11 2024 (in eslint-plugin-react-hooks v5.0.0).
...
* A great resource to check the state of many resources connected or depending on ESLint v9 is the [flat config rollout](https://github.com/eslint/eslint/issues/18093) Issue on GitHub, which shows you which tools completed the transition to ESLint 9 flat config and which are still working on it.

> [!MORE]  
> [ESLint "configuration migrator" issue](https://eslint.org/docs/latest/use/configure/migration-guide#migrate-your-config-file)  
> [ESLint "migration guide" documentation](https://eslint.org/docs/latest/use/configure/migration-guide)  
