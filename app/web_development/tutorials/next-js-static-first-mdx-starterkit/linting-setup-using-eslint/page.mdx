...
<ArchivedMessage case="next-js-15-linting-to-16" />
...
* if Next.js detects that you are still using ESLint v8 they automatically set the `ESLINT_USE_FLAT_CONFIG=false` flag, which [disables flat config support in ESLint v8](https://eslint.org/docs/latest/use/configure/migration-guide#start-using-flat-config-files), so that ESLint keeps using the classic (eslintrc) configuration files
...
If you use the latest **create-next-app** (CNA) and choose to use ESLint, then it will install ESLint v9 for you (in Next.js > 15.1, prior CNA versions did still install ESLint v8), or if you use npm run lint (next lint cli) and have no eslint configuration file, then next lint will also install ESLint v9 for you
...
* finally we will enable caching (as would next lint do) and we set the cache file path to a `.eslintcache` file in the root of the project, which is ESLint's own default location and means clearing the `.next` build output does not throw the lint cache away with it, remember to add `.eslintcache` to your `.gitignore`
...
Congratulations 🎉 you just transitioned from classic (RC) configuration files to flat config and we converted our ESLint configuration file into a TypeScript file (eslint.config.ts), on the next page we will extend our setup by adding linting for TypeScript files using typescript-eslint and also by replacing eslint-config-next with our own custom next flat config, which will finally allow us to get rid of the compatibility mode (FlatCompat)
