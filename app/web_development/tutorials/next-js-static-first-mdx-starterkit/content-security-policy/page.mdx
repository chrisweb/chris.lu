...
Let's edit the CSP rules in our `next.config.ts` file and make the **script-src** directive stricter by not using **unsafe-eval** (the CSP Evaluator flagged **'unsafe-inline'**, which we can not remove, but as an exercise we will remove **'unsafe-eval'** to see what a violation looks like), like so:

```js title="next.config.ts" showLineNumbers{87} {6}#special
    // for dev environment enable unsafe-eval for hot-reload
    return `
        ${defaultCSPDirectives}
        font-src 'self';
...
There are also some useful security headers besides CSP headers. Let's add 4 of those security headers to our Next.js configuration.
...
> [MDN "X-Content-Type-Options" documentation](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/X-Content-Type-Options)  
