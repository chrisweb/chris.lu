...
description: Next.js configuration (next.config.(mjs|ts)), typescript next.js 15 configuration file, options overview like react strict mode, powered by header and redirects  - Next.js static MDX blog | www.chris.lu Web development tutorials
...
```js title="next.config.ts" showLineNumbers {11}#special
import type { NextConfig } from 'next'
import { PHASE_DEVELOPMENT_SERVER } from 'next/constants'
...
```js title="next.config.ts" showLineNumbers {12} /typedRoutes/#special
import type { NextConfig } from 'next'
import { PHASE_DEVELOPMENT_SERVER } from 'next/constants'
