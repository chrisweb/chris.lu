---
title: Styling and CSS libraries - Next.js 15 Tutorial
description: Styling and CSS libraries - Next.js 15 static first MDX starterkit | Web development tutorials | www.chris.lu
...
permalink: https://chris.lu/web_development/tutorials/next-js-static-first-mdx-starterkit/styling-and-css
...
Spoiler alert! I will use [CSS Modules](https://github.com/css-modules/css-modules) in this tutorial. Originally the reason was that a lot of CSS libraries had problems with **Server components** when Next.js 13 got first released. Today we use Next.js 15 and a lot of libraries now work well with server components but it is still fun to use CSS Modules, so as this tutorial is based on the previous iteration for Next.js 14 I will keep using CSS Modules. 
...
If you want to use a UI library instead, then maybe have a look at projects like [shadcn ui](https://ui.shadcn.com/) or [Next UI](https://nextui.org/), which is a beautiful UI based on Tailwind CSS
...
Next.js supports [CSS Modules](https://github.com/css-modules/css-modules) out of the box, which means there is NO need to install any extra packages, and CSS Modules are **React Server Components (RSC)** compatible, which these days should be a requirement for CSS libraries that you intend to use with Next.js 15
...
```tsx title="/components/tutorial_examples/UI/Button.tsx" showLineNumbers /.reset/#special
...
```tsx title="/app/(tutorial_examples)/styling/page.tsx" showLineNumbers /UIButton/#special
...
There is not much to see yet, but if you want to check out the result, make sure your dev server is running (or start it using`npm run dev`) and then in your browser navigate to the styling page at `http://localhost:3000/styling`
...
> [!TIP]  
> For [naming](https://github.com/css-modules/css-modules/blob/master/docs/naming.md) classes, the CSS modules team recommends using camelcase
>  
...
![chrome dev tools elements tab showing how CSS modules class names look after compilation](../../../../../public/assets/images/app/web_development/tutorials/next-js-static-first-mdx-starterkit/chrome_dev_tools_inspect_css_modules.png)
...
* CSS modules allow you to do composition by using the `composes` keyword, this works like an import in JavaScript and allows you to include one css module into another css module
...
> [CSS Modules "composition" documentation](https://github.com/css-modules/css-modules/blob/master/docs/composition.md)  
> [CSS Modules "naming" documentation](https://github.com/css-modules/css-modules/blob/master/docs/naming.md)  
...
```tsx title="/components/tutorial_examples/UI/Button.tsx" showLineNumbers /'use client'/
'use client'
...
```css title="/components/tutorial_examples/UI/button.module.css" showLineNumbers {14-16}
.reset {
    background: none;
    border: none;
    padding: 0;
...
Lines 55 to 62: we make the bottom space a bit smaller and also change the **flex-flow** of the `<main>{:html}` element to **column**, which means that now all elements inside of it, like our `<article>{:html}` and `<aside>{:html}` elements will get displayed one below each other; our surrounding `@media all and (max-width: 768px){:css}` [media query](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_media_queries/Using_media_queries) makes sure we apply these changes only for devices with a viewport smaller than the maximum width of 768px
...
```tsx title="/app/layout.tsx" showLineNumbers {1} {2} {17-23}
import './global.css'
...
>  
> ![chrome settings appearance menu to change the default font-size](../../../../../public/assets/images/app/web_development/tutorials/next-js-static-first-mdx-starterkit/chrome_settings_appearance_change_font_size.png)
...
> [MDN "Flexbox" documentation](https://developer.mozilla.org/en-US/docs/Learn_web_development/Core/CSS_layout/Flexbox)  
...
<Pagination
    ...
    next={{ label: 'Navigation and next/link', href: '/web_development/tutorials/next-js-static-first-mdx-starterkit/navigation-and-next-link' }}
/>
