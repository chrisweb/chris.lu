---
title: Optimizing images in MDX using next/image - Next.js 15 Tutorial
description: Optimizing images in MDX using next/image (LCP Core Web Vital optimization / VSCode MDX image drag and drop) - Next.js 15 static first MDX starterkit | Web development tutorials | www.chris.lu
...
permalink: https://chris.lu/web_development/tutorials/next-js-static-first-mdx-starterkit/optimizing-using-next-image
...
<ArticleJsonLd frontmatter={frontmatter} image="/web_development/og/tutorials_next-js-static-first-mdx-starterkit/opengraph-image" />
...
This page is all about using [next/image](https://nextjs.org/docs/app/api-reference/components/image) in Next.js 15 project to automatically optimize images that we place into our MDX documents. 
...
> [!NOTE]  
> Since Next.js 13, there is a new next/image package, and the previous package got renamed to [next/legacy/image](https://nextjs.org/docs/pages/api-reference/components/image-legacy). The new image component is the result of cooperation between the [Aurora team at Google](https://developer.chrome.com/docs/aurora/image-component) and the Next.js team. The documentation has an informative chapter about ["next/image changed in version 13"](https://nextjs.org/docs/messages/next-image-upgrade-to-13), which explains what has changed and also provides links to codemods to update your codebase automatically  
...
> [MDN "Responsive images" documentation](https://developer.mozilla.org/en-US/docs/Web/HTML/Responsive_images)  
...
> [!NOTE]  
> Although our image is in the `/public/images` folder, we have set the `src` to `/images/playground_test.jpg`, we have omitted the `/public` folder as Next.js will assume that [static assets](https://nextjs.org/docs/app/building-your-application/optimizing/static-assets) like images are in the public folder

If you now look at the code, you will notice that the markdown image element is underlined:

![eslint message "Image elements must have an alt prop, either with meaningful text, or an empty string for decorative images."](../../../../../public/assets/images/app/web_development/tutorials/next-js-static-first-mdx-starterkit/vscode_eslint_modal_missing_image_alt.png 'Image elements must have an alt prop, either with meaningful text, or an empty string for decorative images.')
...
```mdx title="/app/(tutorial_examples)/mdx-components_playground/page.mdx" showLineNumbers{16} /alt/#special
<Image src="/images/playground_test.jpg" alt="Street at night, the wet floor has reflections of the bright lights from multiple billboards and neon signs, one prominent billboard features an advertisement for the TV series Stranger Things 2" />
```
...
```mdx title="/app/(tutorial_examples)/mdx-components_playground/page.mdx" showLineNumbers{16} /width="1200" height="750"/#special
...
![Cache-Control header having max-age set to zero and must-revalidate](../../../../../public/assets/images/app/web_development/tutorials/next-js-static-first-mdx-starterkit/chrome_dev_tools_response_headers_cache.png)
...
```md title="/app/(tutorial_examples)/mdx-components_playground/page.mdx" showLineNumbers {2} {17} /{PlaygroundTestImage}/#special
import Image from 'next/image'
...
>  
> This is because we actually set the src prop of the Next Image component, which will assume your static files are in the **public** folder
>  
...
![browser dev tools inspect element showing an image element with a width and height attribute](../../../../../public/assets/images/app/web_development/tutorials/next-js-static-first-mdx-starterkit/chrome_dev_tools_inspect_elements_image_width_and_height.png)
...
![Cache-Control header having max-age set to high value and immutable](../../../../../public/assets/images/app/web_development/tutorials/next-js-static-first-mdx-starterkit/chrome_dev_tools_response_headers_cache_control_fixed.png)
...
* first, we will install the package, which will add the rehype plugin to our project
...
Now that the plugin is installed, we need to update the Next.js configuration `next.config.ts` file (which is at the root of the project) file to add it to the MDX plugins list

```js title="next.config.ts" showLineNumbers {5} {14} /rehypeMDXImportMedia/#special
...
Line 5: we import the **rehype-mdx-import-media** plugin

Line 14: we add the **rehypeMDXImportMedia** plugin to the **rehypePlugins** list of our MDX configuration
...
```tsx title="/components/base/Image.tsx" showLineNumbers {1} {2} {4} {10}#special {11} {11-13}
import Image from 'next/image'
...
```tsx title="mdx-components.tsx" showLineNumbers {5-6} {15} {31}
...
Lines 5 to 6: we import our custom **BaseImage** component that we just created, and we import the **ImageProps** type from next/image

Line 15: we add a comment to future us know that unlike other mdx components, for the image component we will use the next/image type, which is why we did NOT have to create a custom one ourself

Line 31: we specify that for each markdown image that got transformed into an HTML `<img>{:html}` element, we want to use our custom **BaseImage** component instead
...
```md title="/app/(tutorial_examples)/mdx-components_playground/page.mdx" showLineNumbers {14} /{PlaygroundTestImage}/#special
<article>
...
We have removed the import of **next/image** from the playground MDX document as it is now gets used inside of our `mdx-components.tsx` file, this is great because it means we will NOT have to import next/image into every MDX document in which we have images
...
Then make sure that sure that the VSCode language for your MDX files is set to MDX, you can do that by clicking on the language mode in the bottom right corner of VSCode and then selecting **MDX** from the list of languages
...
![vscode image preview](../../../../../public/assets/images/app/web_development/tutorials/next-js-static-first-mdx-starterkit/vscode_image_preview.png)
...
```md
![Street at night, the wet floor has reflections of the bright lights from multiple billboards and neon signs; one prominent billboard features an advertisement for the TV series Stranger Things 2](../../../public/images/playground_test.jpg)
```
...
```md
![Street at night, the wet floor reflects the bright lights from multiple billboards and neon signs. One prominent billboard features an advertisement for the TV series Stranger Things 2](../../../public/images/playground_test.jpg 'this is the image title')
```
...
> [web.dev "Core Web Vitals" overview](https://web.dev/explore/learn-core-web-vitals)  
...
![Content-Disposition and Content-Type headers for a webp image](../../../../../public/assets/images/app/web_development/tutorials/next-js-static-first-mdx-starterkit/chrome_dev_tools_response_headers_content_type_and_disposition.png)
...
Lines 49 to 52: we add a new entry for images to our configuration and set the `format` option value to `['image/avif', 'image/webp']`
...
![Content-Disposition and Content-Type headers for an avif image](../../../../../public/assets/images/app/web_development/tutorials/next-js-static-first-mdx-starterkit/chrome_dev_tools_response_headers_content_type_image_avif.png)
...
Lines 40 to 49 (and 56 to 57): we check if the type equals **banner**, and if it does, we set the **priority** property on the image, if it does not match any specific type we just create a default Next image (you can later customize the default image to your liking)

> [!NOTE]  
> We can later extend this feature to check for other types; by adding more checks for new keywords
>  
> Like for example, if an image is of type **header**, we could decide to set the **fill** property as well as the CSS [object-fit](https://developer.mozilla.org/en-US/docs/Web/CSS/object-fit) property to **cover**, what keywords you choose for the types and what you do based on a type is up to you
...
Line 14 (at the very end): we added the `{ banner }` keyword (surrounded by curly brackets) to our image so that our **BaseImage** component knows that it should add the **priority** property.
...
![chrome dev tools elements tab showing the image modal box](../../../../../public/assets/images/app/web_development/tutorials/next-js-static-first-mdx-starterkit/chrome_dev_tools_inspect_image_modal.png)
...
{/* spellchecker:disable-next-line */}
* next, the browser looks at what image sources we offer by looking at the **srcset** sources list, it attempts to find a URL for an image width that is equal to or bigger than the 1088 pixels image it needs and finds an entry with a **condition descriptor** that is **1200w**, so it will use the image URL `/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fplayground_test.50052472.jpg&amp;w=1200&amp;q=75`

{/* spellchecker:disable-next-line */}
The steps are the same for a screen with a higher density, but the result is not. Let's assume this time, the **device pixel ratio** is **2**, so the browser does the math again and finds out it needs an image source that is 1088 x **2** = 2176 pixels wide. Hence, it looks at the srcset again and the image source URL that is equal or bigger than 2176 pixels is `/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fplayground_test.50052472.jpg&amp;w=3840&amp;q=75`, which is the URL for the source with a width of 3840 pixels.
...
We open our `next.config.ts` and change it to this:

```js title="next.config.ts" showLineNumbers{45} {4}
images: {
    // file formats for next/image
    formats: ['image/avif', 'image/webp'],
...
>  
> ![developer tools throttling feature](../../../../../public/assets/images/app/web_development/tutorials/next-js-static-first-mdx-starterkit/chrome_dev_tools_network_throttling.png)  
>  
...
Something worth knowing about the placeholder is that next/image will only create a [blurDataURL](https://nextjs.org/docs/app/api-reference/components/image#blurdataurl) (blurDataURL is an attribute that next/image sets for the image to know what blur image it should use) for images that you import statically. It will NOT create a placeholder for remote images. For example if the image source URL is a remote image hosting service or a CDN then next/image will not be able to create a placeholder. Also next/image does NOT (as of now) create placeholder images for animated images, like an animated gif, animated webp or animated avif won't have a placeholder.
...
<Pagination
    previous={{ label: 'Optimizing links in MDX using next/link', href: '/web_development/tutorials/next-js-static-first-mdx-starterkit/optimizing-using-next-link' }}
    next={{ label: 'MDX plugins', href: '/web_development/tutorials/next-js-static-first-mdx-starterkit/mdx-plugins' }}
/>
