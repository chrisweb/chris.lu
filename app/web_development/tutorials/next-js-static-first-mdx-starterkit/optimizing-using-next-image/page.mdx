...
```js title="next.config.ts" showLineNumbers {4} /rehypeMDXImportMedia/#special
...
Line X: we add a comment to future us know that unlike other mdx components, for the image component we will use the next/image type, which is why we did NOT have to create a custom one ourself
