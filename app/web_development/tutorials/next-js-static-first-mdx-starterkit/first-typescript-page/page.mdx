...
We just saw that when using the `npm run dev` command, Next.js will automatically create a root layout file, this is however only the case when you have removed the `--turbopack` from the dev script (in our package.json) as we did in a previous [Typescript plugin and typed routes](/web_development/tutorials/next-js-static-first-mdx-starterkit/typescript-plugin-and-typed-routes) chapter

If you use the `npm run dev-turbo` command (or if you prefer, use `npm run dev -- --turbopack`) and have no root Layout, then Next.js will **NOT** automatically create one for you 😔.
...
<Pagination
    previous={{ label: 'Next.js configuration', href: '/web_development/tutorials/next-js-static-mdx-blog/next-config' }}
