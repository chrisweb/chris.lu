...
The easiest way to stop the development server is to press `Ctrl+C` (the same shortcut on Windows, macOS and Linux)

On Windows you will then get asked if you want to quit:

> Terminate batch job (Y/N)?

To confirm, either enter `Y` and then press `ENTER` or just press `Ctrl+C` again
