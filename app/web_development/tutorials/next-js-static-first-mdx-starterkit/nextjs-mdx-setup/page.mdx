...
In Next.js 15 if you try to use MDX (remark / rehype) plugins and have mdxRs enabled then they will silently fail to load. This is because the MDX Rust compiler can NOT read plugins written in Javascript.

This means you must chose between having faster builds (mdxRs: true) or if you prefer having support for plugins (mdxRs: false)

Next.js 15.1 adds support for MDX plugins, but only if those plugins have serializable options, we will see how to use and when not to use this new plugins loader in the upcoming [page about MDX plugins](/web_development/tutorials/next-js-static-first-mdx-starterkit/mdx-plugins)

> [!MORE]  
> [chris.lu "MDX (rehype / remark) plugins" page](https://nextjs.org/docs/app/api-reference/config/next-config-js/mdxRs)  

## @mdx-js/loader instead of mdxRs

If you do NOT use the MDX rust compiler (mdxRs) because the option is set to false, or because it is commented out, like in the following configuration:
...
As we are NOT using the Rust compiler we now need to install the MDX webpack plugin using the following command:
