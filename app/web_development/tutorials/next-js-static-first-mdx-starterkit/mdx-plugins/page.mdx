...
You might have heard the word "MDX plugins" before, these **MDX plugins are either remark, rehype or recma plugins** and in the next chapter we will see why there are 3 flavors of plugins

MDX plugins are a great way to add support for new features into your existing MDX setup, for example in this tutorial we will use one MDX (rehype) plugin in the [Optimizing images](/web_development/tutorials/next-js-static-first-mdx-starterkit/optimizing-using-next-image) chapter to turn the path of our images into imports, we will add a [Code highlighting plugin](/web_development/tutorials/next-js-static-first-mdx-starterkit/code-highlighting-plugin) (with VSCode themes support), a [Table of Contents](/web_development/tutorials/next-js-static-first-mdx-starterkit/table-of-contents-plugin) plugin, a [github like alerts](/web_development/tutorials/next-js-static-first-mdx-starterkit/github-like-alerts-plugin) plugin and several more
...
In Next.js 15.1 the Next.js team added a plugins loader written in JavaScript, to add support for MDX plugins when using Turbopack with mdxRs disabled. There is a catch though, for it to work the plugin options need to be serializable
...
| -----                                                               | -----                                                                              | -----                                                                          |
| [solution 1](#solution-1-nextconfigmjs-with-turbopack-but-no-mdxrs) | [solution 2](#solution-2-nextconfigmjs-with-plugins-but-no-turbopack-and-no-mdxrs) | [solution 3](#solution-3-nextconfigts-with-mdxrs-and-turbopack-but-no-plugins) |

#### Solution 1 (next.config.mjs with Turbopack, but NO mdxRs)

If you use Turbopack, then make sure your MDX plugins have serializable options and use the plugin name instead of an import. Also make sure mdxRs is disabled, because as soon as mdxRs is enabled @next/mdx will use its mdx-rs-loader, which means the string plugins loader (that Next.js 15.1 added for Turbopack compatibility) does NOT get used and your plugins get ignored
...
> [!TIP]  
> **Update:** disabling Turbopack is no longer the only option, there is now a way to keep Turbopack AND use plugins whose options are NOT serializable (functions), by moving those plugin configurations into local wrapper modules. I wrote a separate tutorial about it: [Next.js 16 Turbopack with MDX plugins that have function options](/web_development/tutorials/next-js-16-turbopack-mdx-plugins)
...
#### Solution 3 (next.config.ts with mdxRs and Turbopack, but NO plugins)
