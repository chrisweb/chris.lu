...
permalink: https://chris.lu/web_development/tutorials/next-js-static-first-mdx-starterkit/project-setup-and-first-commit
...
You now have two options, either you install the **latest** version, in which case npm will download the stable version of Next.js (which is good to build something that is intended to go into production soon), or you can install the canary version which is more experimental but it has the advantage that features like PPR are enabled. Of course you can start with canary version while you experiment and then at some point switch to stable (for example by using the [Version Lens](https://marketplace.visualstudio.com/items?itemName=pflannery.vscode-versionlens) VSCode extension)
