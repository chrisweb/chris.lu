---
title: Table of contents (TOC) plugin - Next.js 15 Tutorial
description: Table of contents (TOC) remark plugin (remark-table-of-contents) - Next.js 15 static first MDX starterkit | Web development tutorials | www.chris.lu
...
permalink: https://chris.lu/web_development/tutorials/next-js-static-first-mdx-starterkit/table-of-contents-plugin
...
```js title="next.config.mjs" showLineNumbers {9} {35-44} {50} /[remarkTableOfContents, remarkTableOfContentsOptions]/#special
...
```md title="/app/(tutorial_examples)/toc_playground/page.mdx" showLineNumbers {38} /%toc%/#special
<article>
...
Line 32: we use React [Children](https://react.dev/reference/react/Children) to transform the children prop into a flat array of elements. By using [toArray](https://react.dev/reference/react/Children#children-toarray), we also make sure the children will always be an array of ReactNodes
...
```ts title="mdx-components.tsx" showLineNumbers {6} {28-48}
...
<Pagination
    previous={{ label: 'Rehype slug plugin to add IDs to headings', href: '/web_development/tutorials/next-js-static-first-mdx-starterkit/headings-id-plugin' }}
    next={{ label: 'GitHub flavored markdown plugin(s)', href: '/web_development/tutorials/next-js-static-first-mdx-starterkit/github-flavored-markdown-plugin' }}
/>
