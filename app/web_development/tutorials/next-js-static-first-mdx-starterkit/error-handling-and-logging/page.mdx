...
> [!NOTE]  
> The version (15.0.3-canary.1) that I used in my overrides is currently the latest canary, however by the time you read this I'm sure that newer versions will be available, so you should update that version manually, to know which versions are the latest (either for the stable or the canary branch) you can have a look at the [npm versions page for Next.js](https://www.npmjs.com/package/next?activeTab=versions)  
>  
...
## Update sentry version (to use alpha or beta)

If you use the wizard it will always use the **latest** version of Sentry for Next.js

If you want to try out the latest alpha or beta versions, but don't know what versions are currently available, then have a look at the [Sentry for Next.js version](https://www.npmjs.com/package/@sentry/nextjs?activeTab=versions) page
