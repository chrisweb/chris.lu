...
There is the **remark-gfm plugin** which adds GitHub Flavored Markdown (GFM) syntax support to remark (turning that syntax into HTML is then done by remark-rehype), the remark-gfm plugin will add the same features to your MDX pages that GitHub has introduced in their own GitHub Flavored Markdown (GFM), for example, it has support for **autolink literals**, **footnotes**, **strikethrough**, **markdown tables**, **tasklists**, ...
...
Line 55: we add an array containing the **remarkGfm** plugin as well as the **remarkGfmOptions** options object (we just created) to the **remarkPlugins** array
...
const BaseCheckbox: React.FC<React.InputHTMLAttributes<HTMLInputElement>> = (props) => {
    ...
    return (<input onChange={changeHandler} checked={isChecked} {...newProps} />)
