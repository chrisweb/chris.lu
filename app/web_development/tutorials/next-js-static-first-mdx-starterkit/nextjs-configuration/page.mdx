...
```js title="next.config.mjs" showLineNumbers {2} {4} {7}#special
...
```js title="next.config.mjs" showLineNumbers {11}#special
import type { NextConfig } from 'next'
import { PHASE_DEVELOPMENT_SERVER } from 'next/constants'
...
```js title="next.config.mjs" showLineNumbers {12} /typedRoutes/#special
import type { NextConfig } from 'next'
import { PHASE_DEVELOPMENT_SERVER } from 'next/constants'
...
```js title="static redirect example" {18-26}
import type { NextConfig } from 'next'
import { PHASE_DEVELOPMENT_SERVER } from 'next/constants'
