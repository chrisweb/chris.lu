...
> [!NOTE]  
> There is a major change in Next.js 15, which is that the **next.config** file can now have a **.ts** extension (and by default has when creating a project using CNA and using the `--typescript` flag as we did in the previous [Project setup and first commit](/web_development/tutorials/next-js-static-mdx-blog/project-setup-and-first-commit) tutorial page), in Next.js 14 all **next.config** files were still a Javascript file with an **.mjs** extension (but if you prefer to keep using mjs files this is fine too)
...
Then commit your `package.json` and `README.md` with a commit message that explains that we removed turbo from dev script but also created a new dev-turbo script as backup that uses the turbo flag, then sync your latest changes with your remote git provider
...
<Pagination
    previous={{ label: 'Project setup and first commit', href: '/web_development/tutorials/next-js-static-mdx-blog/project-setup-using-CNA-and-first-commit' }}
    next={{ label: 'Next.js configuration', href: '/web_development/tutorials/next-js-static-mdx-blog/next-config' }}
/>
