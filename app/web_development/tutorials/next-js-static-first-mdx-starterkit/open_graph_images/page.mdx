---
title: Open Graph Image(s) - Next.js 15 Tutorial
description: Open Graph Image(s) - Next.js 15 static first MDX starterkit | Web development tutorials | www.chris.lu
keywords: ['Open Graph', 'opengraph', 'OG image', 'Satori', 'vercel/og', 'next/og', 'dynamic opengraph image']
published: 2024-12-10T21:22:23.444Z
modified: 2024-12-10T21:22:23.444Z
permalink: https://chris.lu/web_development/tutorials/next-js-static-first-mdx-starterkit/open_graph_images
...
> [Vercel "OG image examples" documentation](https://vercel.com/docs/functions/og-image-generation#examples)  
...
We need 2 static assets to compose our og image, a **font** and a **background image**, as font we will use the same font we used in our `layout.tsx` for the [next/font examples](/web_development/tutorials/next-js-static-first-mdx-starterkit/navigation-styling-and-next-font), which is **Kablammo** from google fonts

To get the font visit the google fonts [Kablammo](https://fonts.google.com/specimen/Kablammo) page, and then click on the **Get font** button. On the next page, click on the **Download all** button (if the button is not there, first click on the **shopping bag icon** in the top right of the screen). Next, go into the public folder of your project and create a new `/fonts` folder. Now go into your download folder and unzip the font package (which you just downloaded from google fonts). Open the `/static` folder, and then copy the `Kablammo-Regular.ttf` file into the `/public/fonts` folder.
...
As image, we will NOT reuse the one we downloaded earlier in the ["How to **NOT** add images to an MDX document"](/web_development/tutorials/next-js-static-first-mdx-starterkit/optimizing-using-next-image#how-to-not-add-images-to-an-mdx-document) part, because it is very heavy (>2.25MB) (and this time we won't have next/image optimize the image), a font that is >2.25MB would be too heavy (see cache error above). 
...
If you use [dynamic route](https://nextjs.org/docs/app/building-your-application/routing/dynamic-routes) for your page then you will also want to have dynamic opengraph images
...
export default async function Image(props: OpenGraphImageProps) {
...
TODO: params are async in next 15.1
...
TODO: did change the code so need to rewrite the explanation
...
Lines 6 to 20: we use the Next.js [generateMetadata](https://nextjs.org/docs/app/api-reference/functions/generate-metadata) function to get the title from the params; we then check if it is in our allow list and if it is valid we create the metadata

Lines 28 to 41: we do the same "is in allow list" check again; if valid we use the title variable in our JSX; if invalid we use the notFound function from next/navigation to create a 404 response

Make sure the dev server is running, and then you can visit the `http://localhost:3000/blog/posts/foo` page and look at the dynamic opengraph image meta tags inside of `<head>{:html}` element, and to see the image itself visit `http://localhost:3000/blog/posts/foo/opengraph-image/blog_opengraph`

Now that you know how to create dynamic versions, you could even go further, if for example you use similar code to the what we used to make our [sitemap dynamic](/web_development/tutorials/next-js-static-first-mdx-starterkit/sitemap_and_search_console#dynamic-sitemap). You could read the content of MDX files, extract the frontmatter and then use the frontmatter values for your dynamic opengraph images
...
<Pagination
    previous={{ label: 'Metadata (for tsx and mdx pages)', href: '/web_development/tutorials/next-js-static-first-mdx-starterkit/metadata' }}
    next={{ label: 'Frontmatter plugin', href: '/web_development/tutorials/next-js-static-first-mdx-starterkit/frontmatter-plugin' }}
/>
