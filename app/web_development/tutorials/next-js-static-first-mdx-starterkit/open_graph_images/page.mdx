...
export async function generateImageMetadata({ params }: OpenGraphImageProps) {
    ...
    return [
        {
            id: 'blog_opengraph',
            alt: `${(await params).title} | example.com`,
            size: size,
...
Lines 11 to 29: we use the Next.js [generateMetadata](https://nextjs.org/docs/app/api-reference/functions/generate-metadata) function to get the title from the params; we then check if it is in our allow list and if it is valid we create the metadata
