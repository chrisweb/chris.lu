...
Now that we have converted our classic configuration to a flat configuration, let's open the new `eslint.config.mjs` file and have a look at what is inside:
...
We can see that the extends that was in our `.eslintrc.json` file got transformed to a new line that uses the extends of the compatibility layer it has created using the `FlatCompat` class from the `@eslint/eslintrc` package
...
As we just converted our ESLint configuration file into a Typescript file, the next step is to make use of types

W edit the `eslint.config.ts` file and make the following few changes:
...
## Ignore folders and setting linter options
