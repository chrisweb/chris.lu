...
### Config type from ESLint VS typescript-eslint

An alternative to using the type from **typescript-eslint** is to use the [type from the ESLint package](https://eslint.org/docs/latest/use/configure/configuration-files#typescript-configuration-files) as we did in the first examples:
