...
> [!NOTE]  
> In this tutorial we will use the [eslint-plugin-import](https://www.npmjs.com/package/eslint-plugin-import) package (as does eslint-config-next), however there is also [eslint-plugin-import-x](https://www.npmjs.com/package/eslint-plugin-import-x), which is another popular choice you might want to try out 
>  
...
Lines X to X: we add the 5 plugins to the config (we use an eslint disable comment as 3 plugins don't have types)
