...
Lines F to G: Then we add some parser options, this configuration is needed if you use one of the "type checked" sets (if are NOT using **TypeChecked** rules, then you can delete those lines from your configuration)

Lines F to G: And finally we set which file extensions we want to include, for this tutorial we will enable linting for regular typescript and typescript react files
...
Line D: we use the type from the @typescript-eslint utilities as type for our default export
...
> [!WARN]  
> I usually never recommend to remove a component or disable a feature of a framework unless really necessary, as doing so will increase your maintenance costs and your technical debt counter will go up
>  
> However in this case though, by using the **eslint-config-next** component we are applying rules (from several plugins it uses) to all files (even MDX files) and this will pose a problem when later we start adding linting for our markdown content, for example if we would keep using **eslint-config-next** it would attempt to use the typescript-eslint parser, but we want our MDX files to be parsed using a remark parser (which we will create using the MDX ESLint package)
>  
> The problem with removing **eslint-config-next** is that, if in the future they add another great plugin to their recommend linting setup, then we will NOT benefit from the update (as we are not using their package anymore) and we will have to maintain our implementation by manually backporting their update into our own ESLint configuration
...
As we are attempting to reproduce what **eslint-config-next**, this means that we are installing dependencies that we already in the project, the reason we still use it is because by using the @latest tag on each of them we make sure our package.json will contain the latest versions for each of them (before we start using them)
