...
When using [typescript-eslint](https://typescript-eslint.io/users/configs/) you get several different `tseslint.configs.*` you can choose from, the **recommended** config contains a set of recommended rules (this is what Next.js uses), there is also **recommendedTypeChecked** which adds recommended rules that require type information; Next we have **strict** and **StrictTypeChecked**, which you could use instead of the recommended rules, these sets include rules are more opinionated (which is why some projects don't use them, however they can help catch bugs so I recommend giving it a try, you can still disable them later by switching back to **recommended** rules or you could disable some of the rules by setting them to off)

Then we have to choose between two more sets of rules, the **stylistic** set (which is the one I will use in this tutorial) contains rules that do NOT impact program logic but focus on enforcing "simpler code patterns", which means they are even more opinionated than the recommended rules, and then there is **stylisticTypeChecked** the second option, which adds a few more stylistic rules that require type information
...
* TypeScript users, note that we also type our config array using the **FlatConfig.Config[]** type from the @typescript-eslint utilities, which lets TypeScript validate the shape of each config object and catch typos in the keys we use
