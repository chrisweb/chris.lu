...
```shell
npm i eslint-plugin-import@latest eslint-import-resolver-typescript@3.7.0 eslint-plugin-react@latest eslint-plugin-react-hooks@latest eslint-plugin-jsx-a11y@latest --save-dev --save-exact
```

> [!WARN]  
> I have set the version for the **eslint-import-resolver-typescript** to **3.7.0** (instead of latest like for the other packages), because version 3.8.0 introduced a breaking change that prevents aliases in MDX to be resolved correctly
>  
> ```shell
> error Unable to resolve path to module '@/components/ExampleComponent' import/no-unresolved
> ```
>  
> I have opened a [ticket (#367)](https://github.com/import-js/eslint-import-resolver-typescript/issues/367) for the issue but there is no PR / fix yet
