...
This "Next.js 15 static first MDX starterkit" tutorial is about adding MDX support to a Next.js 15 project because MDX pages can be great to create a **developer portfolio**, a **documentation website**, a **blog** and a lot more. The tutorial goes beyond adding MDX support, as it contains several chapters about improving security, developer experience (DX) and optimizations that will improve your web vital metrics. We will use Vercel for our deployment (CI/CD and hosting) needs, but feel to replace that part with a deployment on GitHub pages (setting Next.js output to export) or you could self host your project (Next.js 15 comes with some improvements for self hosting).

Static first means that we will focus on creating a static base, where we use static site generation (SSG) to generate our pages at **build time**, to avoid Server Side Rendering (SSR) at **run time** as much as possible. Pre-rendering (parts of) your pages at built time (when deploying), will minimize the amount of SSR and lead to decreased loading times as well as decreased load on your servers. Beyond this tutorial, I recommend looking into Next.js 15 features like **Partial Pre-rendering (PPR)** (experimental) and **component streaming** (in Next.js 15 using React 19 suspend) to add in dynamic parts, but also **Incremental Static Regeneration (ISR)**.
