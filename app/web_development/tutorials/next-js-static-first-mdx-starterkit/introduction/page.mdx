...
* Support for ESLint **flat config** files: yes in Next.js there is support for ESLint 9, but as we will see [Not everything has full ESLint v9 & flat config support / compatibility](/web_development/tutorials/next-js-static-first-mdx-starterkit/linting-setup-using-eslint#not-everything-is-eslint-v9-and-flat-config-yet), ~~CNA v15.0 for example still installs ESLint v8~~ this is fixed if you use CNA version >=15.1 (CNA now installs ESLint v9), so now everything is ESLint v9 compatible, but both Next.js ESLint packages still use classic configuration files (RC) and NOT flat config, which means you need to use the FlatCompat mode if you want to use them as is
