...
After that step the Sentry.io Wizard will give you a **SENTRY_AUTH_TOKEN** string, if you use a CI/CD for your deployments, for example if you set up a custom GitHub action then you will want to add that environement variable to your GitHub secrets and if you use another CI/CD service you should check out their documentation to learn how to automatically upload sourcemaps to Sentry, the CI/CD tool can authenticate itself using the **SENTRY_AUTH_TOKEN** environment variable and then use the Sentry.io API to automatically upload the sourcemaps of your build to Sentry.io (so that later if there is a bug report on Sentry.io it will be able to use the sourcemaps instead of the minified build files, to show you where the error occured).
...
Next we do as they suggest and start the development server using the command and then we visit their example page in our browser at [http://localhost:3000/sentry-example-page](http://localhost:3000/sentry-example-page)
...
First there is the `next.config.mjs` where you can configure options related to your Next.js project, by default the options that the wizard set for us are good enough, but as soon as you have the time I recommend checking out the [Extend your Next.js Configuration](https://docs.sentry.io/platforms/javascript/guides/nextjs/manual-setup/#extend-your-nextjs-configuration) documentation which explains what each option does
...
```ts title="sentry.client.config.ts" showLineNumbers {3-9} {15} {20} {24} {35}
...
line 3-9 I added two variables for the **replaysOnErrorSampleRate** and the **tracesSampleRate** and set them to zero by default, this is to ensure that no logging happens when not in production, then line 6-9 I added a check to verify if the current environment is **production** and if it is I tell Sentry to always make a replay if there is an error (be careful with this option, in the free plan you only have 50 replays per month), I also set the traces sample rate to 0.1, meaning 10% of the traces will get sent to Sentry, you may want a lower or higher value depending on what plan you are on and then check if you reach your limits or not and then adjust over time

Line 24 I completly disabled **replaysSessionSampleRate**, meaning there will be no replays being made when there is no error, the free plan has 50 replays per month and I prefer to keep all the replays for cases where there is a bug

Line 35 I pass the environment to Sentry, meaning Sentry will know if the environment is preview or production (that value is based on the Vercel environment on which Next.js got deployed)
