...
modified: 2024-08-18T19:56:56.444Z
...
> [!WARN]  
> To enable **reactComponentAnnotation** is usually a good idea as it makes reports more redable by using component names instead of long selectors, but to make this feature happen Sentry needs to add a data attribute to components, this does usually not pose a problem except for **React three fiber** which does not like those extra attributes at all, which means that **React component annotations** are great unless you use **React Three Fiber**
>  
...
>  
> It is only after I had opened an [Issue #13413](https://github.com/getsentry/sentry-javascript/issues/13413) in the **sentry-javascript** repository that I found another [Issue #530](https://github.com/getsentry/sentry-javascript-bundler-plugins/issues/530) in the **sentry-javascript-bundler-plugins** repository, which has a comment by one of the Sentry SDKs mainteners mentioned that they consider adding more options in the future to let you exclude components, however as of now those options are not available yet
...
> [!WARN]  
> If there is a problem with your allowed domains, then you might start seeing 403 errors (Failed to load resource: the server responded with a status of 403) for requests to sentry.io (or your tunnel URL), you might also notice that the error in body of the response is like this:
>  
> `{detail: "event submission rejected with_reason: Cors"}{:shell}`
>  
> This error messages mentions Cors, even though the error gets thrown by the allowed domains sentry filter, this is a known problem an there is a ticket to resolve this issue [#9856](https://github.com/getsentry/sentry-javascript/issues/9856)
> 
> So if you encounter this error have a look at your CORS settings but also double check your allowed domains list
>  
