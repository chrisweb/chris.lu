...
You can either use the Sentry Wizard or follow their manual [setup toturial](https://docs.sentry.io/platforms/javascript/guides/nextjs/manual-setup/), I chose to use the Wizard as it guides you through setting up Sentry and will also install some test files to ensure everything is working, if you chose manual you may want to copy the DSN they give you at the end of the page (a Sentry DSN is an API key you will use when setting up your SDK)
...
**Are you using Sentry SaaS or self-hosted Sentry?** you probably want to chose **Sentry SaaS (sentry.io)** like I did (but if you are a company though you might want to have a look at the hosted version) then press `Enter`

**Do you already have an account?** chose **Yes** (if you did follow the previous chapter or already had an account before) then press `Enter` (or chose **No** if you have no account yet and follow the [account creation](#create-an-account-sign-up) process)

Then the sentry wizard will ask you to login, which will open the sentry login page in your default browser, now login into your account, then go back to your terminal
...
Now wait for the backend and frontend errors to appear (this can take a few minutes, which is a good time to refresh your cup of coffee ☕ (or what ever beverage you prefer))
...
Before we commit / sync all the changes Sentry.io did in our project, **delete** the `app\sentry-example-page\` folder including the `page.jsx` **test page** and the also delete the `app\api\sentry-example-api\` folder inclusing the `route.js` **API test route** file that Sentry.io created to test the error logging, you will not need them anymore
...
There is however one option in the server configuration that is commented out that you might want to consider, the option lets you use the [Spotlight js](https://spotlightjs.com/) package by Sentry. If you want to use it I recommend checking out the documentation to [install and setup Spotlight in a Next.js project](https://spotlightjs.com/setup/nextjs/)
...
On [May 13 2024 Sentry released v8](https://github.com/getsentry/sentry-javascript/releases/tag/8.0.0) of their Javascript SDK, version 8 now makes use of the [Next.js "Instrumentation"](https://nextjs.org/docs/app/building-your-application/optimizing/instrumentation) to integrate monitoring and logging tools into a Next.js app, if after using the Wizard and the starting the development server (using npm run dev) you get the following message(s) in your terminal (command line tool):
...
Then you need to do a little bit of manual work to fully upgrade to **Sentry v8** and support **Next.js "Instrumentation"**, this because the Sentry Wizard has not yet been fully updated yet, it will create a instrumentation.ts file but it will import the two configuration file instead of actually putting the content of those 2 configuration files into the instrumentation.ts file

First open the `instrumentations.ts` file in the root of your project and then open the 2 Sentry configuration files `sentry.edge.config.ts` and `sentry.server.config.ts` (that are also in the root of your poject) as well
...
One final step is to delete both `sentry.edge.config.ts` and `sentry.server.config.ts` files as they are not needed anymore

That's it, congratulations you just upgraded to Sentry v8 🎉
