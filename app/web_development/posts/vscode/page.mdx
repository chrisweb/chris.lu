...
**Option 3**: using the via the VSCode terminal / or your preferred command line tool
...
Congratulations 🎉 you now have a local copy of your GitHub repository, if you liked this post please consider making a [donation](https://buymeacoffee.com/chriswwweb) ❤️ as it will help me create more content and keep it free for everyone
...
## VSCode markdown (and MDX) related tips

Here are a few tips which can make your life easier when editing markdown (like README.md files) or MDX content
...
All you need to do is select the file in the explorer on the right, then drag it over the MDX (or markdown) document, then press `Shift` and drop the file into the document where you want it to be displayed, VSCode will automatically generate the mardown image syntax for you and add the correct path

> [!TIP]  
> to select the image and be able to drag it into the document, click on the image using the left mouse button and **keep it pressed**, then move the file over the document and then release the mouse button when at the position in the document where you want to insert it, if you just do a single click on the image then it will open the image in preview window
...
Also good to know, [VSCode version 1.89 (april 2024)](https://code.visualstudio.com/updates/v1_89#_image-previews-in-markdown-path-completions) added a new feature that allows you to preview images and videos, just hover over the image path and VSCode will show a preview

### VSCode fix for markdown highliting in mdx files

Depending on what theme you use, markdown highliting will be broken when viewing the content of MDX files

This is due to a problem where not all themes are up to date and have colors set for MDX, for example I use the [SynthWave '84](https://marketplace.visualstudio.com/items?itemName=RobbOwen.synthwave-vscode) which I like a lot, unfortunatly all my content in MDX files is just getting displayed used the standard text color
...
If you have already set custom settings for your VSCode workspace then you will have a `.vscode` folder in your project root, if not create that folder, then inside of that folder you will have a `settings.json` file, if that file is not there create it
...
When opening an MDX file you will notice that there is no preview icon in the top left, this is because VSCode seperates markdown from MDX, there is however a trick to make it work, which is to set the language service to markdown for the .mdx extension:

(Same as we did in the previous chapter) we need to edit the `.vscode/settings.json` file and add the following setting for **files.associations**:
...
Now save the settings file and open an .mdx file and you should have the preview icon

> [!NOTE]  
> Of course in the preview you will only see markdown and HTML getting previewed, it will however NOT preview React components you might have imported in your MDX document, for that we need to wait for a VSCode MDX preview feature, but even though React components are not getting displayed I like the preview feature to quickly check if my markdown syntax is ok and maybe you do too 😉

### VSCode markdown headings links

To add links to headings inside a document just use the default syntax, meaning you start with square braquets (`[]`) and between them you add the link text (`[LINK_TEXT]`), then after the square braquets you use add regular braquets (`()`) for the inline link URL or a relative path (`(https://example.com)`), as path start with a number sign (`#`) and a modal will open listing all your headline IDs, just chose the ID you want the link to point to and it will get inserted for you

You can also link to headings in other documents, to do so just follow the previous steps but instead of adding a single number sign (`#`) inside of the regular braquets (`()`) you add 2, like this (`(##)`) this will again open a modal but this time it will not only list the headings IDs of the current file but also those it found in other files

#### renaming headings (links)

To change a heading and update all the links that point to it, just click inside of the heading or link path and then press `F2`, it will open a modal with the current heading title, modify it and then press `ENTER`
...
You can of course use the terminal and type any git command you want, but you can also do a lot of things by using the VSCode command palette
...
> [!TIP]  
> To rename another branch (other than the current one) first switch to the branch you want to rename
