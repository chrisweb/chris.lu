...
* Open VSCode
* On the welcome screen click on **Open Folder**, if you have no welcome screen, you can also on top click on **File** and then **Open Folder**, another option to open a folder is to use the keyboard shortcut `Ctrl+O{:shell}` (macOS: ⌘O, Linux: Ctrl+O)
...
The easiest way to save a file in VSCode is to use the keyboard shortcut `Ctrl+S{:shell}` (macOS: `⌘S{:shell}`, Linux: `Ctrl+S{:shell}`) another way is to click on **File** in the top navigation and then **Save**
...
then press `ENTER{:shell}` and you are done
