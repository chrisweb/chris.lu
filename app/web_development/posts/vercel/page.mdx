...
Chose the radio radio button **Specific Projects** and then **select the project(s)** for which you want to enable the sentry integration

Have a look at the permissions you are giving to Sentry and if that's ok with you click on **Install**

This will open a Sentry.io modal to **connect your projects**, in the first select box chose your project on Vercel and in the second select box your project on Sentry.io and finally click on the **+** (plus sing) button to add a new connection

Then click on **Complete on Vercel** button to get back to Vercel, now you are done, you can come back to the [Sentry integration page on Vercel](https://vercel.com/integrations/sentry) later and make some configuration adjustements or eventually remove the integration
