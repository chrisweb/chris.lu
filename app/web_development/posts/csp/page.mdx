...
My personal recommendation is set the default-src to **none**, meaning that it will disallow any source for any content and then check which violations get listed in your console log, now one by one use resource directives to fix the violations, for example a policy to allow images from the same origin as the html document would look like this:
