...
I saw countless CSP examples (for beginners) that would set the **default-src** to **'self'**, my personal recommendation is set the default-src to **none**, meaning that it will disallow any source for any content type that is not explicitly whitelisted using a fetch directive
