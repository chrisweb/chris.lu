---
title: Node.js
description: My Node.js notes - installing Node.js, checking which version you run, verifying that your version is still receiving security support, and the available alternatives
...
<ArticleJsonLd frontmatter={frontmatter} image="/web_development/og/posts_node-js/opengraph-image" />
...
Under the hood, Node.js uses the Chrome (chromium) [V8 engine](https://nodejs.org/en/learn/getting-started/the-v8-javascript-engine) to execute your JavaScript, but V8 is only the JavaScript engine, the asynchronous, event-driven model of Node.js comes from [libuv](https://libuv.org/), the library that provides its event loop and non-blocking I/O. Still, there are differences. If you would like to learn more about those differences, check out the Node.js article [Differences between Node.js and the Browser](https://nodejs.org/en/learn/getting-started/differences-between-nodejs-and-the-browser).
...
> [!TIP]  
> Node.js >= 22.6.0 [supports TypeScript](https://nodejs.org/api/typescript.html) but you need to use the **--experimental-strip-types** flag, which only strips the type annotations. For TypeScript only features, like enums or namespaces, you additionally need the **--experimental-transform-types** flag, which got added in Node.js >= 22.7.0. Node.js > 23.6.0 has type stripping enabled by default. If you want to use an older Node.js version then you will need to first compile the TypeScript code to JavaScript or you convert the code to JavaScript manually.
