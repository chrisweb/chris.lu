...
Why use React and Next.js (as a beginner), simply because both [React](https://reactjs.org/) and [Next.js](https://nextjs.org/) are very popular in the javascript community, based on the latest results from the **state of js survey** that got released in january 2023 and is based on a survey they did at the end of november 2022. This time almost 40k developers from around the world participated and based on their feedback react is in 2022/2023 the most used frontend framework: ["state of js 2022 survey results: frontend frameworks"](https://2022.stateofjs.com/en-US/libraries/front-end-frameworks/) and next.js is the most used rendering framework ["state of js 2022 survey results: frontend frameworks"](https://2022.stateofjs.com/en-US/libraries/rendering-frameworks/).

Because React and Next.js are very popular it means that there are a ton of tutorials on the web for you to learn how to use those technologies, also because of their huge communities you will find an abundance of pre made tools and components you can use in your projects, which means less writing things yourself, which is good because it speeds up creating a new project and also because there are big communities behind those packages they are often feature rich and well tested, last but not least learning to use a popular framework means that you will find a lot of open positions on the job market.

> [!NOTE]  
> the latest [state of js survey 2023](https://survey.devographics.com/en-US/survey/state-of-js/2023) just happend not long ago in december 2023 but as of now the results are not out yet, I will update this post as soon as they however I don't expect much change in regards to the popularity of React and Next.js
...
The app directory and server side React are all features you can opt in when ready, but nobody forces you to use them. The pages directory still exists in Next.js 13 and 14 and React is still has all the client components features it had before the introductions of Server Components and Server Actions.

This means you can still start a new project today and use the pages directory (I however don't recommend it), if you have an existing codebase you don't need to migrate everything at once from pages to the app directory, you can do it bit by bit, however if you start a new project in 2024 I recommend that you consider using the app directory as well as Server Components and eventually Server Actions, because those technologies are more future proof and also include a lot of great features the pages directory or client side only projects written in React did not have until today
...
Not so long ago all Next.js only had the **pages** directory, since the first release of Next.js 13, Next.js now has two main directories: the **pages** and **app** directories:

* when using the **pages** directory, files in the pages directory automatically become available as routes, which means that the file structure directly corresponds to the URL structure. For example, pages/foo.tsx becomes example.com/foo
* the **app** directory in Next.js offers a more advanced approach for managing layouts, styles, and complex routing patterns. For example, pages/foo/page.tss becomes example.com/foo, because **foo** is now a directory and not the name of the page file, this allows us to add other files in that directory like a layout.tsx file to manage the styling of the page and all others pages in sub-directories, a loading.tsx file to add a loader that gets displayed while the content is being fetching
...
> [!MORE]  
> [Next.js blog post "App Router common mistakes and how to fix them"](https://vercel.com/blog/common-mistakes-with-the-next-js-app-router-and-how-to-fix-them)  
> [Next.js blog post "Server Components and Server Actions Security"](https://nextjs.org/blog/security-nextjs-server-components-actions)  
> [Next.js "App Router Incremental Adoption Guide" documentation](https://nextjs.org/docs/app/building-your-application/upgrading/app-router-migration)  
