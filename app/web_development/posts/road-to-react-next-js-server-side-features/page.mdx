...
**Server Components** (in my opinion) make it easier to get data from an API compared to using the getServerSideProps in pages (less framework specific syntax you need to know), a regular fetch will do (fetch in Next.js got extended with framework specific features like a caching mechanism, but in the end it still works like a regular fetch), here is a very simple example:
