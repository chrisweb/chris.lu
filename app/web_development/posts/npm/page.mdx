...
The beta will have no impact on your **latest** package, meaning that if users use the regular `npm i PACKAGE_NAME` command they will still get the version tagged latest, however if a user wants to **explicitly install the beta** they can do so using this command:
...
In [march 2024 Deno announced jsr.io](https://deno.com/blog/jsr_open_beta), [JSR](https://jsr.io/) is an ESM only (and typescript first) package registry , check out the **JSR** [docs](https://jsr.io/docs/introduction) if you want to learn more about **publishing** or **installing** packages, also good to know is that [JSR has a npm compatibility layer](https://jsr.io/docs/npm-compatibility)
