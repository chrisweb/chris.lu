...
It is also possible to use a Git(Hub) url of a private repository, there are just a few more steps needed to achieve that

First you need to create a **deploy key** and add it to your GitHub repository, to learn how to create and use deploy keys check out the chapter ["Create a deploy key to access a private repository" in my GitHub post](/posts/github#create-a-deploy-key-to-access-a-private-repository)

Now that the SSH key has been created and is ready to be used, add your private repository to the `package.json` like this:
