...
**Turbopack + MDX plugins**: Turbopack requires the MDX plugin config in `next.config.ts` to be JSON-serializable (plugin names as strings, options as plain objects). Plugins whose options need JS values (the custom github-alerts `build` function, shiki transformers, the autolink-headings `properties` callback) or that lack a default export live in wrapper modules under `lib/mdx/` that export a `[plugin, options]` tuple, referenced by absolute path. Don't move those options back inline into `next.config.ts` — that breaks the Turbopack build. Editing a `lib/mdx/` wrapper requires a dev-server restart (they're loaded by the MDX loader, not watched). The `build` script is plain `next build` — Turbopack is the default since Next.js 16, no flag needed. `@next/mdx` does add a `webpack` function to the config, and Next bails when it defaults to Turbopack and finds a `webpack` config with no `turbopack` config (`lib/turbopack-warning.js`, `process.exit(1)`) — but @next/mdx also sets `nextConfig.turbopack` whenever `process.env.TURBOPACK` is set, so that guard never fires here. Building with `--webpack` is broken anyway on Next 16.2+ (MDX metadata false-positive, vercel/next.js#91735).
