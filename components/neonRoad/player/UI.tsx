...
// Move mixtape data outside component to avoid effect initialization issues
const MIXTAPE = [
    {
        source: [
            {
...
const PlayerUI = forwardRef<PlayerCore, unknown>((_, playerRef) => {
    ...
    // Initialize credits state directly from MIXTAPE to avoid effect setState
    const [creditsState, setCreditsState] = useState<ICredits>(() => {
        const firstSong = MIXTAPE[0]
        return {
            name: firstSong.name,
