...
* I could write an article explaining server vs client components, explaining the 'use client' and 'use server' directive, that if no directive by default a page, layout or component is a server component, also tell that components can be in the app directory among pages (which was not the case with pages router) but also explain I prefer to still keep my components in a components directory outside of the app directory, link to this page <https://nextjs.org/docs/app/building-your-application/rendering>, also add an example where the component has no directive but we add an event handler or try to use state and then get told to convert it to a client (make same test with a component outside of the app directry, no use client directive but with an event handler, should see the error that it can't be a server component !?), explain that sometimes it is better to not just make everything a client but maybe some code can be moved to achieve that the least amount of components are client components, also explain that having client components does not mean they don't get prerendered on the server too, mention hooks don't need a use client directive, also add examples for these 3 things: A) server components can NOT get IMPORTED into client components, you ALWAYS need to pass them as children prop from within a server component down to the client component that will use it: <https://nextjs.org/docs/app/building-your-application/rendering/composition-patterns>, B) a component with no statement, a component located into a segment directory (TODO: verify, only if in segment directory? I think this is just always true) if loaded by a client component will also be considered being a client component without having to explicitly add the 'use client' statement on top, C) example of server component, show it printing a message in the terminal, then re-use it but his time being imported into a client component and it just works (no need to add 'use client' statement) and show that now the message appears in the browser console and not the server anymore
* in mdx chapter, add example for using extensions other than .mdx <https://github.com/vercel/next.js/blob/canary/packages/next-mdx/readme.md>